...
#define MCI_PCM_BUF_SIZE        (4608) // 9KB

UINT32 pcmbuf[MCI_PCM_BUF_SIZE];

UINT32 *inputbuf_overlay = pcmbuf;
...
UINT32 audio_file_input_stream_alloc( INT32 file, audio_input_stream_struct* input_stream )
{
	if(input_stream)
	{
	    ...
	    input_stream->buf = inputbuf_overlay;
...
UINT32 audio_input_stream_read( audio_input_stream_struct *input_stream, UINT32 size )
{
	...
	app_trace(1,"audio_input_stream_read,read_data_size:%d,file_handle:0x%x,free_size:%d",read_data_size,input_stream->file,free_size);
...
BOOL id3_tag_parse_v2_tag_header( UINT8* data, id3_tag_struct*id3_tag)
{
	...
	id3_tag->v2_size = id3_tag_get_unsync_uint32( data + 6, 4 ); // 6, 7, 8, 9
  	...
  	app_trace(1,"[v2_tag_header]v2_size:%d,version:%d,flags: %d,data[0-2]:%c,%c,%c",id3_tag->v2_size,version,flags,data[0],data[1],data[2]);
...
BOOL id3_tag_parse_v2_frame_data( UINT8* data, id3_tag_frame_header_struct* frame_header, id3_tag_struct* id3_tag)
{
    ...
    app_trace(1,"[id3_tag_parse_v2_frame_data,String]:%s,text_size:%d,frame_id_index:%d.",src,text_size,frame_id_index);
...
BOOL id3_tag_parse_v2_tag( audio_input_stream_struct* input_stream, id3_tag_struct* id3_tag)
{
	...
	if( id3_tag_parse_v2_tag_header( ptr, id3_tag) )
	{
		...
		{
			app_trace(1,"id3_tag_parse_v2_tag,v2_tag_size: %d,buf_size:%d",v2_tag_size,buf_size);
		...
		else
		{
			...
			app_trace(1,"id3_tag_parse_v2_tag,else,v2_tag_size:%d,line is %d.",id3_tag->v2_size,__LINE__);
...
*     
*****************************************************************************/
BOOL id3_tag_parse(CONST UINT32 file , audInfoStruct *aud_info)
{
	INT32 data_size;
