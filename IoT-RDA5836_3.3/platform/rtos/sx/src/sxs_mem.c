...
void sxr_InitCluster   (void)
{
    ...
    if(InitDone ==  FALSE)
    {
        ...
        sxr_Heap = sxr_HMalloc((PAL_NB_RAM_AREA + SXR_NB_MAX_POOL + SXR_NB_HEAP_USER)
                                *sizeof(HeapCtx_t));
#else
        sxr_Heap = sxr_HMalloc((PAL_NB_RAM_AREA + SXR_NB_HEAP_USER)
                                *sizeof(HeapCtx_t));
#endif
        sxr_Cluster = sxr_HMalloc(SXR_NB_MAX_POOL*sizeof(ClusterCtx_t));
