...
void sxr_TaskInit()
{
    sxr_Task.Ctx = sxr_IntMalloc(SXR_NB_MAX_TASK*sizeof(sxr_TaskCtx_t));
}
...
void sxr_RunNextTask (void)
{
    ...
    {
#if 1//ndef __SXS_TRACE_ENABLE__ Print stack overflow msg always
        hal_DbgAssert("Stack overflow %lx %s", sxr_Task.Ctx [NewPendingTask].Sp, sxr_Task.Ctx [NewPendingTask].Desc -> Name);
#else
