...
void *_sxr_HMalloc (u32 Size, u8 Idx)
{
	...
	HBlock = COS_Malloc(Size + sizeof(sxr_HMemHead_t));
	memset(HBlock, 0, sizeof(sxr_HMemHead_t));
	HBlock ->H.Link      = 1;
	sxr_SetCheckSum (&HBlock -> H.CheckSum);
    sxr_ExitSc (Status);
...
void _sxr_HFree (void **Ptr)
{
	sxr_HMemHead_t *HBlock = (sxr_HMemHead_t *)( ((u8 *)*Ptr) - sizeof (sxr_HMemHead_t));
	u32 Status = sxr_EnterSc ();
...
{
	//COS_PreAlloc(Size, NbCluster);
}

#if 0//def __SXR_CLUSTER__
// =============================================================================
// sxr_Malloc
// -----------------------------------------------------------------------------
