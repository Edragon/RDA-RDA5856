...
u8 sxs_FlashInit (void)
{
 ...
 if(InitDone == FALSE)
 {
    sxs_FileMaxSize = sxr_HMalloc(SXS_FLH_NB_FILES_MAX*sizeof(u16));
    sxs_FileOffset = sxr_HMalloc(SXS_FLH_NB_FILES_MAX*sizeof(u32));

    sxs_FileMaxSize[0] = 20 * SXS_FLH_NB_FILES_MAX;
    for(i=0; i< SXS_FLH_NB_FILES_MAX; i++)
    {
        sxs_FileMaxSize[i] = 512;
    }
...
void sxs_FlashPurge (u8 Protect, u8 UpdtStatic)
{
 ...
 || (sxs_FlashCtx.PurgeRequired))
 {
  ...
  u32 *FilesStart = sxr_HMalloc(SXS_FLH_NB_FILES_MAX*sizeof(u32));
  u32 *FilesEnd = sxr_HMalloc(SXS_FLH_NB_FILES_MAX*sizeof(u32));
  u32 i,j;
  u16 *BankStart [3];
  u8  NbBank = 1;
