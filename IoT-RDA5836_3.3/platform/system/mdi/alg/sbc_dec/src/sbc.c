 ...
 */
static int sbc_unpack_frame(uint8_t *data, struct sbc_frame *frame,
				size_t len)
{
	int consumed;
				...
				struct sbc_frame *frame, int ch, int blk)
{
	...
	for (i = 0; i < 8; i++) {
		...
		v[offset[i]] = 
			((synmatrix4[i][0] * frame->sb_sample[blk][ch][0])
			+(synmatrix4[i][1] * frame->sb_sample[blk][ch][1])
			+(synmatrix4[i][2] * frame->sb_sample[blk][ch][2])
			+(synmatrix4[i][3] * frame->sb_sample[blk][ch][3]) ) >>15;
	}
	...
	for (idx = 0, i = 0; i < 4; i++, idx += 5) {
		...
		frame->pcm_sample[ch][blk * 4 + i] = 
			((v[offset[i] + 0] * sbc_proto_4_40m0[idx + 0])
			+(v[offset[k] + 1] * sbc_proto_4_40m1[idx + 0])
			+(v[offset[i] + 2] * sbc_proto_4_40m0[idx + 1])
			+(v[offset[k] + 3] * sbc_proto_4_40m1[idx + 1])
			+(v[offset[i] + 4] * sbc_proto_4_40m0[idx + 2])
			+(v[offset[k] + 5] * sbc_proto_4_40m1[idx + 2])
			+(v[offset[i] + 6] * sbc_proto_4_40m0[idx + 3])
			+(v[offset[k] + 7] * sbc_proto_4_40m1[idx + 3])
			+(v[offset[i] + 8] * sbc_proto_4_40m0[idx + 4])
			+(v[offset[k] + 9] * sbc_proto_4_40m1[idx + 4])) >>16;
	}
}
				...
				struct sbc_frame *frame, int ch, int blk)
{
	...
	state->V[ch][offset[i]] = ((synmatrix8[i][0] * frame->sb_sample[blk][ch][0])
				+(synmatrix8[i][1] * frame->sb_sample[blk][ch][1])
				+(synmatrix8[i][2] * frame->sb_sample[blk][ch][2])
				+(synmatrix8[i][3] * frame->sb_sample[blk][ch][3])
				+(synmatrix8[i][4] * frame->sb_sample[blk][ch][4])
				+(synmatrix8[i][5] * frame->sb_sample[blk][ch][5])
				+(synmatrix8[i][6] * frame->sb_sample[blk][ch][6])
				+(synmatrix8[i][7] * frame->sb_sample[blk][ch][7])) >>15;
	}
	...
	for (idx = 0, i = 0; i < 8; i++, idx += 5) {
		...
		frame->pcm_sample[ch][blk * 8 + i] = 
			((state->V[ch][offset[i] + 0] * sbc_proto_8_80m0[idx + 0])
			+(state->V[ch][offset[k] + 1] * sbc_proto_8_80m1[idx + 0])
			+(state->V[ch][offset[i] + 2] * sbc_proto_8_80m0[idx + 1])
			+(state->V[ch][offset[k] + 3] * sbc_proto_8_80m1[idx + 1])
			+(state->V[ch][offset[i] + 4] * sbc_proto_8_80m0[idx + 2])
			+(state->V[ch][offset[k] + 5] * sbc_proto_8_80m1[idx + 2])
			+(state->V[ch][offset[i] + 6] * sbc_proto_8_80m0[idx + 3])
			+(state->V[ch][offset[k] + 7] * sbc_proto_8_80m1[idx + 3])
			+(state->V[ch][offset[i] + 8] * sbc_proto_8_80m0[idx + 4])
			+(state->V[ch][offset[k] + 9] * sbc_proto_8_80m1[idx + 4]) )>>16;
	}
}
...
static void _sbc_analyze_four(const int32_t *in, int32_t *out)
{
	sbc_fixed_t t[8], s[5];
...
static  void _sbc_analyze_eight(const int32_t *in, int32_t *out)
{
	sbc_fixed_t t[8], s[8];
...
static void sbc_set_defaults(sbc_t *sbc, unsigned long flags)
{
	...
	sbc->blocks = SBC_BLK_16;//SBC_BLK_4;
	sbc->bitpool = 48;
...
int sbc_init(sbc_t *sbc, unsigned long flags)
{
	...
	sbc->priv = COS_Malloc(sizeof(struct sbc_priv));
	if (!sbc->priv)
		return -ENOMEM;
