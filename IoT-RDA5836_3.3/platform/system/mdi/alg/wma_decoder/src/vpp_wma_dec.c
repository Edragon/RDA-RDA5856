...
void vpp_WmaSetBandInfoEnable(uint16 flag)
{
	*((UINT16 *)hal_VocGetPointer(WMADEC_BAND_INFO_ENABLE_FLAG_ADDR)) = flag;
}
#ifdef MUSIC_BAND_INFO

void vpp_WmaCleanBandInfo(void)
{
    UINT16 i;
    ...
    UINT32 *pBandVoCInfo;
        ...
        pBandVoCInfo = (UINT32 *)hal_VocGetPointer(m_WMADEC_BAND_INFO_ADDR);
...
void vpp_WmaGetBandInfo(UINT16 * band_buf, UINT16 ch)
{
    UINT32 band[8];
    UINT16 shift, i;
	...
	hal_HstSendEvent(SYS_EVENT,0x20171315); 
	hal_HstSendEvent(SYS_EVENT,ch); 

        pBandVoCInfo = (UINT32 *)hal_VocGetPointer(m_WMADEC_BAND_INFO_ADDR);
	 *(INT16 *)hal_VocGetPointer(WMADEC_BAND_INFO_ENABLE_FLAG_ADDR) = 1;
	 shift = 2;
	if(ch == 2)
	{
	    shift += 1; 
...
HAL_ERR_T vpp_WmaGetInfo( int32 *pStreamIn ,VPP_WMA_DEC_CONTENT_T * pWmaProperty )
{
	...
	tWMAFileStateInternal *WMAFileState = NULL;
    	uint8 *pchar;
    HAL_ERR_T cwma_result;
    
	WMAFileState = (tWMAFileStateInternal*)COS_Malloc(sizeof(tWMAFileStateInternal));
	...
	if(cWMA_NoErr == iResult)
    {
        ...
        cwma_result = HAL_ERR_NO;
	}
	else
	{
	    cwma_result = HAL_ERR_RESOURCE_NOT_ENABLED;
	}

    if(WMAFileState != NULL)
    {
        COS_Free(WMAFileState);
    }
	return cwma_result;
}
...
HAL_ERR_T vpp_WmaDecOpen(HAL_VOC_IRQ_HANDLER_T vocIrqHandler, int32 * pStreamIn,int32 *output, int16 * PrevOutput) 
{
	...
	diag_printf("[VPP_WMA]Opening VPP WMA Decoder\n"); 
 ...
 //	if (WMA_OPEN_VOC_NO == ifOpenVoc)		return HAL_ERR_NO;
	...
	if(!WeightingMode||fNoiseSub)
		...
		if(WeightingMode)
			...
			if(fNoiseSub)
				{
				    cfg.vocCode              = (INT32 *)wma_code_mid;
			...
			{
				    cfg.vocCode              = (INT32 *)wma_code_low;
		...
		{ 
		    cfg.vocCode              = (INT32 *)wma_code_high;
...
#ifdef MUSIC_BAND_INFO
      vpp_WmaSetBandInfoEnable(1);
#else
      vpp_WmaSetBandInfoEnable(0);
#endif
...
void vpp_WmaDecClose(void)
{
   ...
   //g_play_disk_WMA_ERROR=1;
    diag_printf("[VPP_WMA] Closing VPP");
}
...
HAL_ERR_T vpp_WmaDecScheduleOneFrame(/*INT32 * pStreamIn,*/ INT32 * pStreamOut, UINT8 EQMode)
{
    ...
    g_last_buffer_addr = (UINT16*)pStreamOut;
...
//	*(INT32*)hal_VocGetPointer(GLOBAL_FILE_POINTER_ADDR) = (INT32)hal_VocGetDmaiPointer(pStreamIn,HAL_VOC_DMA_READ,HAL_VOC_DMA_BURST,HAL_VOC_DMA_B2S_NO);
    ...
    *((INT32*)hal_VocGetPointer(GLOBAL_Digital_gain_ADDR)) = algGain;
