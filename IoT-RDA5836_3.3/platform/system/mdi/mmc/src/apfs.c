							...
							MCI_ProgressInf* PlayInformation)
{
    ...
    {
        result =  MMC_WavGetFileInformation (  FileHander,  (AudDesInfoStruct*)&info  );
    }
    
    if(FileType == MCI_TYPE_DAF || FileType ==  MCI_TYPE_AAC)
    {
         mmc_EntryMode(MMC_MODE_AUDIO, FileType);
         result = Audio_MpegGetFileInformation(FileHander, (AudDesInfoStruct  * CONST)&info, FileType);
    }
#ifndef TARGET_WMA_SUPPORT_OFF
    else if(FileType == MCI_TYPE_WMA)
    {
        hal_HstSendEvent(SYS_EVENT,0x88000741);
         mmc_EntryMode(MMC_MODE_AUDIO, FileType);
        result = Audio_WMAGetFileInformation(FileHander, (AudDesInfoStruct  * CONST)&info);
    }
#endif
