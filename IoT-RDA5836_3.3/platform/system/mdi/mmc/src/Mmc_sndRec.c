...
static UINT32 g_audioRecSampleRate = 0;
...
extern UINT16 FMR_Get_SampleRate(void);
...
INT32 sndRec_writeWaveData(HANDLE fhd, UINT8 *pcm_data, UINT length)
{
    ...
    #else
    memcpy(sndRecRingBuffer,pcm_data,length);
    result=FS_Write(fhd, sndRecRingBuffer, length);
    #endif
...
INT32 Mmc_MicStartRecord(UINT32 *pBuffer, UINT32 nBufferSize,ARS_REC_MODE_T codec,BOOL loop_mode,ARS_USER_HANDLER_T recHanlder)
{
    ...
    if (g_audioRecSampleRate == HAL_AIF_FREQ_8000HZ) {
...
INT32 Mmc_sndRecStart(HANDLE fhd,U8 quality, mci_type_enum format,UINT32 samplerate,MCI_AUDIO_FILE_RECORD_CALLBACK_T callback,  MCI_AUDIO_RECORD_BUFFER_PLAY_CALLBACK_T usercallback)
{
	INT32 result = 0;
	U8 snd_quality;
//	UINT8 waveHeader[WAVE_FILE_HEADER_SIZE]={0};
	...
	sndRecBuffer=(INT32*)pcmbuf_overlay; //(INT32 *)mmc_MemMalloc(SNDRECBUF_LEN*4);
	sndRecRingBuffer=(INT8*)inputbuf_overlay;//(INT32 *)mmc_MemMalloc(SNDRECBUF_LEN*2);


    g_SndRecUserHandler = usercallback;
    g_audioRecSampleRate = samplerate;
	mmcRecFormat = format;
	switch(mmcRecFormat)
	{
		case MCI_TYPE_WAV_DVI_ADPCM:
			...
			{
				sndRecBufferLength = (fhd > 0 && fhd < SNDRECBUF_LEN) ? fhd : SNDRECBUF_LEN;
			}
			else
			{
     ...
     if(g_RecordPlayOpen==1)
     	{
		    ...
		    if(part == SNDREC_DATA_PART_HALF)
		    {
			   ...
			   //hal_HstSendEvent(SYS_EVENT, 0x10702530);
                        result = sndRec_writeWaveData(recHandle, (UINT8*)sndRecBuffer, sndRecBufferLength*2);
            			}
		            }
		...
		if(g_SndRecUserHandler != NULL)
		{
		    g_SndRecUserHandler((UINT8*)(((UINT16*)sndRecBuffer)+sndRecBufferLength), sndRecBufferLength*2, (unsigned int *)&result);
		}
		else
			{ //hal_HstSendEvent(SYS_EVENT, 0x10702560);
                        result = sndRec_writeWaveData(recHandle, (UINT8*)(((UINT16*)sndRecBuffer)+sndRecBufferLength), sndRecBufferLength*2);
			}
		            }
