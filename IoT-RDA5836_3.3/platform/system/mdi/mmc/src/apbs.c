              ...
              else  //ADPCM
              {
               ...
               if((g_apbsAdpcmBufRead!=NULL )&&(g_apbsAdpcmRemainSize>0))
                  ...
                  while(Remain_pcm>0)
                  {
                           ...
                           length = adpcm_decode_frame(input, (void*)g_apbsAdpcmBuf,(INT32 *)&data_size, (UINT8 *)g_apbsBufferCurPos ,input->block_align);
    ...
    BOOL loop)
{
	...
	hal_HstSendEvent(SYS_EVENT,(UINT32)0xaccc2222);
	hal_HstSendEvent(SYS_EVENT,(UINT32)g_apbsBufferForLoopmode_alloc);
	hal_HstSendEvent(SYS_EVENT,(UINT32)g_apbsBufferForLoopmode);
#endif
    switch (stream->mode)
    {
        //HAL_SPEECH_DEC_IN_T decStruct;
...
//	        MCI_TRACE(MCI_AUDIO_TRC|TSTDOUT,0,"[APBS] apbs_GetWriteBuffer: g_apbsBufferCurPos = %x g_apbsBufferRemainingBytes=%d\n", CurPos, RemaingBytes);	
             *buffer =(UINT32*)((UINT8* ) CurPos+RemaingBytes-g_apbsBufferLength);
	}
	else 
	{
		*buffer= (UINT32*)((UINT8 *)CurPos+RemaingBytes);
	}
