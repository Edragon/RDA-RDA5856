...
// Callback
PRIVATE BOOT_HOST_USB_CALLBACK_T           g_BootHostUsbCallbackOs=0;
    ...
    while(pos + g_BootHostUsbVar.Residue < size)
    {
        ...
        switch(g_BootHostUsbVar.State)
        {
            ...
            case BOOT_HOST_USB_STATE_HEADER:
                ...
                {
                    header                      = (BOOT_HOST_USB_HEADER_T*) &data[pos];//pos=1
            ...
            case BOOT_HOST_USB_STATE_SXS_CMD:
                ...
                g_BootHostUsbVar.Cmd          = *((BOOT_HOST_USB_CMD_T*)&data[pos]);//pos=4
            ...
            case BOOT_HOST_USB_STATE_SXS_DATA:
                ...
                if(g_BootHostUsbVar.Cmd.internal &&               //shaohuima 0924
                   g_BootHostUsbVar.Cmd.rw == BOOT_HOST_USB_CMD_RW_WRITE)
                {
                    // Internal Write
                ...
                else 
                {
                    ...
                    {
#if 0
                    	//mon_Event(0x15092404);
						// Save USB context before we try to copy first block of usb 
						// ramRun bin, for it will definitly flush .data&.bss section.
						// Question: How does this work in 8808/8809?
						if(0xa1c000a0 == g_BootHostUsbVar.Addr)// Added by ZM. 20150925
						{
							UINT32 saveLength = (size-pos)<(0x280-0xa0)?(size-pos):(0x280-0xa0);
							mon_Event(0x15092444);
							mon_Event(saveLength-0x10);
							memcpy((VOID*)(&data[pos+16]), 
							           (UINT32*)0xa1c000b0, saveLength-0x10);
						}
#endif
...
PRIVATE VOID boot_HostUsbSendReadPacket(VOID)
{
    ...
    boot_UsbSend(ENDIN, g_BootHostUsbVar.Context
...
PUBLIC VOID boot_HostUsbInit(VOID)
{
    ...
    //boot_UsbEpEnableInterrupt(HAL_USB_EP_DIRECTION_OUT(2), FALSE);
    boot_UsbRecv(ENDOUT, g_BootHostUsbVar.Context->BufferOut,
...
PRIVATE CONST HAL_USB_EP_DESCRIPTOR_T boot_HostUsbEp1In = {
    .ep        = HAL_USB_EP_DIRECTION_IN(ENDIN),
...
PRIVATE  CONST HAL_USB_EP_DESCRIPTOR_T boot_HostUsbEp1Out = {
    .ep        = HAL_USB_EP_DIRECTION_OUT(ENDOUT),
    .type      = HAL_USB_EP_TYPE_BULK,
    .interval  = 1,
    .callback  = 0
};
...
PUBLIC VOID boot_HostUsbEvent(UINT32 event)
{
    ...
    while(boot_UsbEpTransfertDone(HAL_USB_EP_DIRECTION_IN(ENDIN)) == FALSE)
    {
    ...
    boot_UsbSend(ENDIN, trame, sizeof(trame), 0);
...
PUBLIC VOID boot_HostUsbClockEnable(VOID)
{
	...
	//hwp_sysCtrl->Cfg_Clk_Sys 	= SYS_CTRL_FREQ_52M;

	//mon_Event(0x15102601);
...
#else
	//hwp_sysCtrl->REG_DBG        = SYS_CTRL_PROTECT_UNLOCK;
	hwp_sysCtrl->Clk_Other_Enable = SYS_CTRL_ENABLE_OC_USBPHY;// clk_usbphy

	hwp_sysCtrl->usb_cfg    |= (0x1<<1);// usb_pll_pu

	*(UINT32*)(0xa1a05000) &= 0xfffffff0;// GPIO cfg
	*(UINT32*)(0xa1a05000) |= 6;

	*(UINT32*)(0xa1a05004) &= 0xfffffff0;// GPIO cfg
	*(UINT32*)(0xa1a05004) |= 6;

	hwp_sysCtrl->REG_DBG    = SYS_CTRL_PROTECT_LOCK;

	UINT32 pll_reserved_dig2 = 0xee85; // set USB GPIO highz
	boot_IspiSendData(BOOT_ISPI_CS_USB, pll_reserved_dig2, FALSE);
#endif
}
...
PUBLIC BOOL boot_HostUsbSend(VOID)
{
    ...
    if(boot_UsbEpTransfertDone(HAL_USB_EP_DIRECTION_IN(ENDIN)) == TRUE &&
       g_BootHostUsbVar.PositionBufferIn != 0)
    {
        boot_HostUsbSendReadPacket();
    }

    if(boot_UsbEpTransfertDone(HAL_USB_EP_DIRECTION_IN(ENDIN) == FALSE))
    {
        return(TRUE);
    }
    return(FALSE);
...
PUBLIC BOOT_MONITOR_OP_STATUS_T boot_HostUsbRecv(VOID)
{
    ...
    switch(g_BootHostUsbVar.RecvState)
    {
        case BOOT_HOST_USB_RECV_STATE_RUN:
            if(boot_UsbEpTransfertDone(HAL_USB_EP_DIRECTION_OUT(ENDOUT)))
            {
                ...
                // is in the residue.
                size = boot_UsbEpRxTransferedSize(ENDOUT)+g_BootHostUsbVar.Residue;
                // Decode received packet.
                exitStatus = boot_HostUsbDecode(data, size);//shaohuima
                startNewReception       = 1;
            }
            break;
       ...
       startNewReception == 1)
    {
        ...
        boot_UsbRecv(ENDOUT, &g_BootHostUsbVar.Context->BufferOut[g_BootHostUsbVar.Residue],
