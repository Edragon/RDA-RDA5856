...
PUBLIC VOID boot_AbbIspiWrite(UINT32 regIdx, UINT32 value)
{
	UINT32 wrData;
	...
	//HAL_ISPI_CS_USB == 2 in 5856.
    while(boot_IspiTxFifoAvail() < 1 ||
            boot_IspiSendData(1, wrData, FALSE) == 0);
...
/// Enable USB PHY regs.
// =============================================================================
PUBLIC VOID boot_UsbPHYEnable(VOID)
{
	boot_UsbIspiWrite(0x83, 0x4448);
	boot_UsbIspiWrite(0x89, 0xee85);
