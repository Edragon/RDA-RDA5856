...
PUBLIC VOID hal_UartOpen(HAL_UART_ID_T id, CONST HAL_UART_CFG_T* uartCfg)
{
    ...
    g_halUartPropArray[id-1].rxRequestId = HAL_IFC_UART_RX + 2*(id - 1);
    g_halUartPropArray[id-1].txRequestId = HAL_IFC_UART_TX + 2*(id - 1);
#else
    ...
    case HAL_UART_BAUD_RATE_1843200:
        ...
        #else
            clockToUse = SYS_CTRL_UART_CLK_SEL_52M_XTAL_52M;
        #endif
        break;
    ...
    default:
        ...
        #else
            clockToUse = SYS_CTRL_UART_CLK_SEL_52M_XTAL;
        #endif
        break;
    }
    ...
    uartClockDivisor = fsSys / (mode * fs);
    //#ifndef CHIP_ASIC_ID_5857 | CHIP_ASIC_ID_5856E
    //  Configure the clock register. 

//    hwp_sysCtrl->uart_clk_cfg = SYS_CTRL_UART_CLK_NUM(1) | SYS_CTRL_UART_DIVIDER(uartClockDivisor) |
    hwp_sysCtrl->uart_clk_cfg = SYS_CTRL_UART_CLK_NUM(72) | SYS_CTRL_UART_CLK_DENOM(uartClockDivisor) |
        clockToUse |SYS_CTRL_UART_CLK_DENOM(1016) ;
    //UART clock load configuration
    hwp_sysCtrl->uart_clk_cfg |= SYS_CTRL_UART_CLK_LD_CFG;
    //#endif
#else
    uartClockDivisor = ( (fsSys + ((mode / 2) * fs)) / (mode * fs) ) - 2;
...

    {
        ...
        for(i=0; i<HAL_UART_QTY-1; i++)
        {
			...
			else if (g_halUartPropArray[i].breakIntWakeup)
			{
				...
				//#ifndef CHIP_ASIC_ID_5857
#if (CHIP_ASIC_ID >= CHIP_ASIC_ID_5856E) 
				    hwp_sysCtrl->uart_clk_cfg = SYS_CTRL_UART_CLK_NUM(1) | SYS_CTRL_UART_CLK_DENOM(0) |
					SYS_CTRL_UART_CLK_SEL_52M_XTAL;
                //UART clock load configuration
                    hwp_sysCtrl->uart_clk_cfg |= SYS_CTRL_UART_CLK_LD_CFG;
				//#endif
#else
...
PUBLIC VOID hal_UartSetRts(HAL_UART_ID_T id, BOOL ready)
{
    ...
    if (ready)
    {
        g_halUartHwpArray[id-1]->cmd_set = UART_RX_RTS;
    }
    else
    {
        g_halUartHwpArray[id-1]->cmd_clr = UART_RX_RTS;
    }
}
