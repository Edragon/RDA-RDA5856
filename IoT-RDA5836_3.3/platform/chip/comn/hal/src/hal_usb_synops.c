...
#define ADPLL_SDM_144M  0x2C4EC4EC //268435456

#define RXFIFOSIZE 64
#define TXFIFOSIZE 48
extern PROTECTED BOOL hal_GdbIsInGdbLoop(VOID);
extern VOID DRV_Mcd_RegisterClientDriver();
...
PRIVATE VOID hal_UsbUpdateDivider(HAL_SYS_FREQ_T freq);
...
PRIVATE VOID hal_UsbConfig(VOID)
{
  ...
  //       hwp_usbc->DIEPTXF[2].DIEnPTXF  = USBC_IENPNTXFSTADDR(addr) | USBC_INEPNTXFDEP(1);
	...
	hwp_usbc->GUSBCFG &=~USBC_USBTRDTIM_MASK;
    hwp_usbc->GUSBCFG |= USBC_PHYIF | USBC_USBTRDTIM(9);
...
#ifdef DONGLE_HOST    
    UINT8 device_Descriptor[0x12] = {0x12,0x01,0x10,0x01,0x00,0x00,0x00,0x40,
                                     0x04,0x1e,0x07,0x06,0x01,0x01,0x01,0x02,
                                     0x03,0x01};
    UINT8 configuration_Descriptor[0x20] = {0x09,0x02,0x20,0x00,0x01,0x01,0x00,0x00,
                                            0x32,0x09,0x04,0x00,0x00,0x02,0x08,0x06,
                                            0x50,0x00,0x07,0x05,0x01,0x02,0x40,0x00,
                                            0x00,0x07,0x05,0x81,0x02,0x40,0x00,0x00};
    UINT8 string_Descriptor[0x12] = {0x12,0x03,0x63,0x00,0x6f,0x00,0x6f,0x00,
                                     0x6c,0x00,0x73,0x00,0x61,0x00,0x6e,0x00,
                                     0x64,0x00};
#endif
...
//                 }
            ...
            case HAL_USB_REQUEST_DEVICE_GETDESC:
...
#ifdef DONGLE_HOST
                    memcpy(g_HalUsbBufferEp0In, device_Descriptor, sizeof(device_Descriptor));
                ...
                case 2: /* Config */
                 ...
                 //   hal_HstSendEvent(USB_EVENT, setup->lenght);
#ifdef DONGLE_HOST
                    memcpy(g_HalUsbBufferEp0In, configuration_Descriptor, sizeof(configuration_Descriptor));
                ...
                case 3: /* String */
                    ...
                    {
#ifdef DONGLE_HOST
                        memcpy(g_HalUsbBufferEp0In,string_Descriptor,sizeof(string_Descriptor));
                        size = sizeof(string_Descriptor);
#else
        ...
        case HAL_USB_REQUEST_DESTINATION_INTERFACE:
            ...
            {
                hal_UsbSend(0, (UINT8*)"\0", 1, 0);
            }
//            for(UINT8 i = 0; i < 8; ++i)
//            {
//            //    ((UMSS_BO_TRANSPORT_LUN_T)g_umssBoLun[i]).pos      = 0;
//                hal_HstSendEvent(USB_EVENT, 0x11290002);
//            }
//            //break;
#endif
...
PROTECTED VOID hal_UsbIrqServiceDevice(UINT8 interruptId)
{
    UINT32 status;
    UINT32 statusEp;
    UINT32 data;
...
#ifdef DONGLE_HOST
    hwp_usbc->DAINTMSK         = 3 | (3<<16);
    g_HalUsbVar.EpInterruptMask = 3 | (3<<16);
#endif
...
{
    	//hal_HstSendEvent(USB_EVENT, 0x51251251);
    	//hal_HstSendEvent(USB_EVENT,hwp_usbc->GOTGCTL);
    	if((hwp_usbc->GOTGCTL&USBC_CONLDSTS))
    	{
		return hal_UsbIrqServiceDevice(interruptId);
    	}	
	else
	{
#ifdef MCD_USB_HOST_SUPPORT
		//hal_HstSendEvent(USB_EVENT, 0x14090501);
		return hal_UsbIRQServiceHost(interruptId);
#endif
	}	
}
...
PUBLIC VOID hal_ConfigUsb_Host(BOOL enable)
{
	if(enable)
	{
...
PUBLIC VOID hal_usbinitclockenable(VOID)
{
      // UINT32 i;
	//*****************************************************************//
       //a.open usb_phy_ctrl.pu_usb = 1,b.usb_phy_ctrl.usb_ctrl= 3’b001
       hwp_abb->usb_phy_ctrl &=~(ABB_USB_CTRL_MASK|ABB_PU_USB);
      if(hwp_usb_role_det->irq_status&USB_ROLE_DET_HOST_MODE)
	{
	hwp_abb->usb_phy_ctrl |=ABB_PU_USB|ABB_USB_CTRL(4);
        }
      else
        {
        hwp_abb->usb_phy_ctrl |=ABB_PU_USB|ABB_USB_CTRL(1);
        }	
	//b.usb_phy_ctrl.usb_ctrl= 3’b001
	//hwp_abb->usb_phy_ctrl |=ABB_USB_CTRL(1);
	//c.Gpio8 and Gpio9 to cfg 10(DP,DN) 
	hwp_iomux->cfg[8] = IOMUX_FUNC_SEL(HAL_GPIO_ALTFUNC_CFG_10);
	hwp_iomux->cfg[9] = IOMUX_FUNC_SEL(HAL_GPIO_ALTFUNC_CFG_10);
	//d.dp/dn=0,IO highzu
	hwp_iomux->cfg[8] &=~IOMUX_PDN;
	hwp_iomux->cfg[9] &=~IOMUX_PDN;
    	hal_SysRequestFreq(HAL_SYS_FREQ_USB, HAL_SYS_FREQ_78M, hal_UsbUpdateDivider);
      hal_UsbUpdateDivider(hal_SysGetFreq());	
	//***************************************************//
	//e.Open Sys_c trl.per_module_en.usbc_en (USB CLK)
...
PRIVATE VOID hal_UsbUpdateDivider(HAL_SYS_FREQ_T freq)
{
	...
	//f.Open pmu_intf.xtal_ctrl[xtal_clk_adpll_en_lv]=1(USB PLL)
    hwp_pmu_intf->xtal_ctrl |=PMU_INTF_XTAL_CLK_ADPLL_EN_LV; 
	
	//g.open adpll_setting0[adpll_refmulti2_en] = 0
    hwp_rf_if->adpll_setting0 &=~RF_IF_ADPLL_REFMULTI2_EN;

	//h.hwp_rf_if->adpll_sdm_setting1 = 32'h2C4EC4EC; 对应144M,
	hwp_rf_if->adpll_sdm_setting1 = ADPLL_SDM_144M;

	//i.hwp_rf_if->adpll_sdm_setting0 |=RF_IF_ADPLL_SDM_FREQ_DR;
    // open adpll_sdm_setting0[adpll_sdm_freq_dr] = 1
	...
	//hal_HstSendEvent(0xff, 0x20170415);
	...
	hwp_rf_if->adpll_setting0 |=RF_IF_ADPLL_RSTN_REG;  
    hal_HstSendEvent(0xff, 0x20170413);
	...
	//10. While(!hwp_rf_if->adpll_status.adpll_lock_flag)  //wait pll lock
	//NEW[Xian]: 请检测adpll_status.adpll_lock_det。
     while(!(hwp_rf_if->adpll_status&RF_IF_ADPLL_LOCK_DET));
	//usb hclk快速时钟选择pll div
	//13. hwp_rf_if->adpll_setting1[sys_fast_clk_sel] = 3
	hwp_rf_if->adpll_setting1 &= ~RF_IF_SYS_FAST_CLK_SEL_MASK;
    hwp_rf_if->adpll_setting1 |= RF_IF_SYS_FAST_CLK_SEL(3);
#endif	 
       UINT32 RFIFCtrl = hal_SysEnterRFIFCtrlAPB();
       //HAL_ASSERT(HAL_SYS_PLL_FREQ_144M ==  hal_SysGetAdPLLFreq());
	//11. SYS_CLK_CFG_USBC_PHY_CLK_DIV_DIV_3
	hwp_rf_if->adpll_setting1 &=~RF_IF_USBC_PHY_CLK_DIV_MASK;
	hwp_rf_if->adpll_setting1 |=RF_IF_USBC_PHY_CLK_DIV_DIV_3;	
	//12.adpll_sdm_setting1adpll_sdm_setting1
      hwp_rf_if->adpll_setting1 |=RF_IF_USBC_PHY_CLK_LD_CFG;
      hal_SysExitRFIFCtrlAPB(RFIFCtrl);
    
}

VOID hal_musb_start_synops()
{

    hal_HstSendEvent(0xff, 0x20170315);
#if(CHIP_ASIC_ID >= CHIP_ASIC_ID_5856E)
	// reset USBC shaohuima 
	hwp_abb->usb_phy_ctrl        &=~(ABB_USB_CTRL_MASK|ABB_PU_USB);  
	hwp_abb->usb_phy_ctrl        |=ABB_PU_USB|ABB_USB_CTRL(1);
	hwp_abb->usb_phy_ctrl        &=~ABB_USB_CTRL_MASK;
	hwp_sysCtrl->per_module_en |= SYS_CTRL_USB_DET_EN_ENABLE ;
    //*(UINT32*)(0xa1a00000) &=~(1<<13);
      hwp_sysCtrl->per_soft_reset1  &=~SYS_CTRL_USBC_PHY_RSTB;
      hwp_sysCtrl->per_soft_reset1  &=~SYS_CTRL_USBC_RSTB;
	//*(UINT32*)(0xa1a00000)&=~(1<<7);
	//c.Gpio8 and Gpio9 to cfg 4(DP,DN) 
	hwp_iomux->cfg[8]              = IOMUX_FUNC_SEL(HAL_GPIO_ALTFUNC_CFG_4);
	hwp_iomux->cfg[9]              = IOMUX_FUNC_SEL(HAL_GPIO_ALTFUNC_CFG_4);
	hwp_iomux->cfg[8] |= IOMUX_PDN;
	hwp_iomux->cfg[9] |= IOMUX_PDN;
	//hwp_usb_role_det->irq_clear |=USB_ROLE_DET_ROLE_DET|USB_ROLE_DET_TIMEOUT;
	hwp_usb_role_det->irq_mask    |= USB_ROLE_DET_DET_MASK;              
	hwp_usb_role_det->irq_mask    |= USB_ROLE_DET_TIMEOUT_MASK;
    #if(CHIP_ASIC_ID <= CHIP_ASIC_ID_5856E)
	hwp_sys_irq->mask_set           |= SYS_IRQ_MASK_SET(0x400000);
    #endif
#if 1
	hwp_usb_role_det->irq_mask &= ~USB_ROLE_DET_LOOP_TIME_MASK;          
	hwp_usb_role_det->det_cfg |=USB_ROLE_DET_LOOP_INTERVAL(0);
	hwp_usb_role_det->det_cfg |=USB_ROLE_DET_DET_INTERVAL(0);
	hwp_usb_role_det->det_cfg |=USB_ROLE_DET_WAIT_INTERVAL(0);
#endif
      hal_SysRequestFreq(HAL_SYS_FREQ_USB, HAL_SYS_FREQ_32K, 0);
	hal_HstSendEvent(0xff, 0x2017022a);	
#endif
}

PUBLIC BOOL hal_UsbIsPlugIn(VOID)
{
    return (hwp_iomux->cfg[8] == IOMUX_FUNC_SEL(HAL_GPIO_ALTFUNC_CFG_10));
}
...
{
            COS_EVENT ev;
            hal_HstSendEvent(0xff, 0x20170406);
#if(CHIP_ASIC_ID >= CHIP_ASIC_ID_5856E)
#if 0
            if(hwp_usb_role_det->irq_status&USB_ROLE_DET_TIMEOUT_INT)
            {
            hal_HstSendEvent(USB_EVENT, 0x20170407);
            hwp_usb_role_det->irq_clear |=USB_ROLE_DET_TIMEOUT;
            return ;
            }
#endif
		 hwp_usb_role_det->irq_clear   |=USB_ROLE_DET_ROLE_DET;
              hwp_sysCtrl->per_soft_reset1 |=SYS_CTRL_USBC_PHY_RSTB;
              hwp_sysCtrl->per_soft_reset1 |=SYS_CTRL_USBC_RSTB;
              //***********************************************//
              //hwp_sysCtrl->per_soft_reset1|=SYS_CTRL_USBC_RSTB;
		 //*(UINT32*)(0xa1a00000)|=(1<<7);
		 //*(UINT32*)(0xa1a00000)|=(1<<13);
		 //hal_HstSendEvent(0xff, 0x20170408);
	       //hal_HstSendEvent(0xff, hwp_usb_role_det->irq_status&USB_ROLE_DET_USB_ID);
		 //mon_Event(hwp_usb_role_det->irq_status&=USB_ROLE_DET_USB_ID);
	       //************************************************//
		if(hwp_usb_role_det->irq_status&USB_ROLE_DET_HOST_MODE)// host 
		{
		hal_HstSendEvent(0xff, 0xffffaaaa);
             hal_AbbEnableUsbPhy(TRUE);
		DRV_Mcd_RegisterClientDriver();
		hal_HstSendEvent(USB_EVENT, 0x20170409);
		hal_usbinitclockenable();
		hal_HstSendEvent(USB_EVENT, 0x17010410);
		ev.nEventId = EV_FS_USB_PLUG_DETECT;
		ev.nParam1 = 0;
		COS_SendEvent(MOD_SYS, &ev, COS_WAIT_FOREVER, COS_EVENT_PRI_NORMAL);
             //**************************************************//
		//hal_HstSendEvent(0xff, 0x20170411);
	      //hal_HstSendEvent(0xff, hwp_usb_role_det->irq_status&USB_ROLE_DET_USB_ID);
	      //mon_Event(hwp_usb_role_det->irq_status&=USB_ROLE_DET_USB_ID);
	     //***************************************************//
		}
		if(hwp_usb_role_det->irq_status&USB_ROLE_DET_DEVICE_MODE)// device
		{
		hal_HstSendEvent(USB_EVENT, 0x20170414);
		hal_usbinitclockenable();
             ev.nEventId = EV_FS_USB_PLUG_DETECT;
		ev.nParam1 = 0;
		COS_SendEvent(MOD_SYS, &ev, COS_WAIT_FOREVER, COS_EVENT_PRI_NORMAL);
