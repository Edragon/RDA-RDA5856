...
#define AIF_SOURCE_CLOCK_5857  26000000
...
PUBLIC HAL_ERR_T hal_AifSerialOpen(CONST HAL_AIF_CONFIG_T* config)
{
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_5856) 
		serialCfgReg |= AIF_BCK_LRCK(serialCfg->bckLrckRatio-16);
#endif
        // BckOut gating
    ...
    *(VOLATILE UINT32*)(0xa1a2600c) = 0x04000000;
...
/// @param 
// =============================================================================
PRIVATE HAL_ERR_T hal_AifCfg2Reg(CONST HAL_AIF_CONFIG_T* config, UINT32 *controlReg, UINT32 * serialCfgReg)
{
    UINT32 lrck;
    UINT32 bck;
    UINT32 clkDivider;
    ...
    else
    {
        ...
        // Mode
        serialCfg = config->serialCfg;
        HAL_ASSERT((serialCfg->mode < HAL_SERIAL_MODE_QTY), "Improper serial mode");
        *serialCfgReg |= AIF_SERIAL_MODE(serialCfg->mode);
        ...
        {
            // HAL_AIF_IF_SERIAL or HAL_AIF_IF_SERIAL_IN_PARALLEL_OUT
        ...
        // Rx delay
        HAL_ASSERT(serialCfg->rxDelay<HAL_AIF_RX_DELAY_QTY, "Improper delay for serial Rx");
        *serialCfgReg |= AIF_RX_DLY(serialCfg->rxDelay);
        ...
        // Tx Mode
        HAL_ASSERT(serialCfg->txMode < HAL_AIF_TX_MODE_QTY, "Improper mode for serial Tx" );
        *serialCfgReg |= AIF_TX_MODE(serialCfg->txMode);

        // The ratio is calculated differently depending on whether we're using voice or audio mode.
...
/// @param 
// =============================================================================
PRIVATE HAL_ERR_T hal_AifOpen5857(CONST HAL_AIF_CONFIG_T* config)
{
    UINT32 controlReg = 0;
    UINT32 serialCfgReg = 0;
    UINT32 lrck;
    ...
    clkDivider = AIF_SOURCE_CLOCK_5857 / config->sampleRate - 2;
    hal_SysSetTxStbDiv(clkDivider);

    hal_AifCfg2Reg(config, &controlReg, &serialCfgReg);
      
    hal_SysSetBclkDiv(clkDivider>>5);
...
PUBLIC HAL_ERR_T hal_AifOpen(CONST HAL_AIF_CONFIG_T* config)
{
    ...
    clkDivider = 26000000 / config->sampleRate - 2;
//change pll end

    hwp_sysCtrl->Cfg_Aif_Tx_Stb_Div = SYS_CTRL_AIF_TX_STB_EN | (1<<30)
    ...
    else if (config->interface == HAL_AIF_IF_PARALLEL_STEREO)
    {
     ...
     if (config->channelNb == HAL_AIF_MONO)
        {
            serialCfgReg |= AIF_MASTER_MODE_MASTER | AIF_TX_MODE_MONO_STEREO_DUPLI | AIF_RX_MODE_STEREO_STEREO;
        }
        else
        {
            serialCfgReg |= AIF_MASTER_MODE_MASTER | AIF_TX_MODE_STEREO_STEREO | AIF_RX_MODE_STEREO_STEREO;
        }
     #endif
