...
PUBLIC HAL_ERR_T hal_DmaMemCopy(UINT32 *dest, CONST UINT32 *src, UINT32 n)
{
    ...
    hwp_dma->dst_addr = (UINT32)dest;
    hwp_dma->sd_dst_addr = (UINT32)0x0fffffff;
    hwp_dma->pattern  = 0;
    hwp_dma->xfer_size = DMA_TRANSFER_SIZE(n);
    // Configure the CTRL register and launch the transfer.
    controlReg = DMA_DST_ADDR_MGT_NORMAL_DMA;
...
PUBLIC HAL_ERR_T hal_DmaMemSet(UINT32 *dest,UINT32 c, UINT32 n)
{
    ...
    hwp_dma->src_addr = (UINT32)0x0fffffff;
    hwp_dma->dst_addr = (UINT32)dest;
    hwp_dma->sd_dst_addr = (UINT32)0x0fffffff;
    hwp_dma->pattern  = c;
    hwp_dma->xfer_size = DMA_TRANSFER_SIZE(n);
