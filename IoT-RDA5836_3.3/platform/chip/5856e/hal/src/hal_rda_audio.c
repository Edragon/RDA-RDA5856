...
extern PUBLIC VOID gpio_SetMute(BOOL mute_flag);
...
PUBLIC VOID hal_AudSetHeadGain(UINT32 gain)
{
    ...
    data = ABB_HP_GAIN_BIT(2);

    //Disable headphone when gain is 0
    if(0 != gain) data |= ABB_PA_EN_HP;  // TODO:   Move me to hal_AudSetSelCfg
	
    hal_AbbRegWrite(CODEC_DAC_SETTING2, data);
	
#else
        hwp_abb->DAC_Gain = ABB_HP_GAIN_BIT(gain) |  ABB_MUX_GAIN_BIT(3) | ABB_DAC_GAIN_BIT(1);  
#endif
}

PUBLIC HAL_AUD_PD_T hal_AudGetPowerStatus(VOID)
{
    HAL_AUD_PD_T pdStatus={0};
