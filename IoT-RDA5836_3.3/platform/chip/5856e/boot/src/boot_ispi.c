...
PRIVATE VOID boot_IspiIfcChannelRelease(UINT32 requestId, UINT8 channel)
{
    ...
    if (GET_BITFIELD(hwp_sysIfc->std_ch[channel].std_ch_ctrl, SYS_IFC_REQ_SRC) == requestId)
    {
    ...
    cfgReg =
        ((spiConfigPtr->csPmuActiveLow ? SPI_CS_POLARITY_0_ACTIVE_LOW:
         ...
         | (spiConfigPtr->csFmActiveLow ? SPI_CS_POLARITY_2_ACTIVE_LOW:
                                        ...
                                        //need to check cs counter in SPI 20161011
    #endif                                    
    // Calculate divider.
...
PUBLIC UINT32 boot_IspiSendData(BOOT_ISPI_CS_T csId, UINT32 data, BOOL read)
{
    ...
    freeRoom = GET_BITFIELD(hwp_spi->status, SPI_TX_SPACE);
...
{
    UINT32 ifcTc;
    ifcTc = hwp_sysIfc->std_ch[g_bootIspiProperties.rxIfcCh].std_ch_tc;
    return ifcTc;
}
...
// boot_UsbIspiWrite
// -----------------------------------------------------------------------------
/// Write reg in USB PHY by SPI.
///
/// @param regIdx Reg index to write.
/// @param value value to write.
// =============================================================================
PUBLIC VOID boot_UsbIspiWrite(UINT32 regIdx, UINT32 value)
{
	UINT32 wrData;
    
    wrData = (0<<25) | ((regIdx & 0x1ff) << 16) | (value&0xffff);

	//HAL_ISPI_CS_USB == 2 in 5856.
    while(boot_IspiTxFifoAvail() < 1 ||
            boot_IspiSendData(2, wrData, FALSE) == 0);
