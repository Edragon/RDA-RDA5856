...
UINT8 exfat_GetSecNumPerClus(UINT64 iDiskTotSec)
{
    UINT64 iDiskToSize;
	UINT32 i;
	UINT8 Powerof2;
	iDiskToSize = iDiskTotSec * 512;
    ...
    //disks larger than 32 GB, 128KB or more
	for(i = 17; ; i++)
	{
...
UINT32 exfat_GetFATSz(UINT64 iTotalSec,
	               UINT32 iFatOffset,
                   UINT32 iSecPerClus)
{    
    UINT32 iFATSz;
...
INT32 exfat_read_root_dir(UINT32 DevNo,UINT64 SecNum,UINT32* BATClus,UINT32* UCTClus, UINT32* UCTTable)
{
    INT32 result;
...
/*
    ...
    */
    ...
    CSW_TRACE(BASE_DSM_TS_ID, "debug EXFAT:boot_rec:\n"
            "PartitionOffset = 0x%llx,\n",
            boot_rec.PartitionOffset
            );
    if(SecNum != boot_rec.PartitionOffset)
    {
        D((DL_FATERROR, " iSecNum = 0x%x,PartitionOffset = 0x%llx",SecNum,boot_rec.PartitionOffset));
        CSW_TRACE(BASE_DSM_TS_ID, " iSecNum = 0x%x,PartitionOffset = 0x%llx",SecNum,boot_rec.PartitionOffset);
        iRet = _ERR_FAT_BPB_ERROR;
        goto end;
    }

    if(boot_rec.ValuemLength < (UINT64)(value_64<<20) || boot_rec.ValuemLength > (UINT64)((value_64<<63) -1))
    {		
