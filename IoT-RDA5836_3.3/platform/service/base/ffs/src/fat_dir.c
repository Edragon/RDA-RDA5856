...
INT32 fat_trunc_fatentry(FAT_SB_INFO *sb_info, UINT32 lFstCluLO, BOOL isRemove)
{
    ...
    do
    {
        ...
        if(!DSM_IsRemoveableDev(sb_info->sb->s_dev))
        {
    ...
    if(!isRemove)
    {
        lErrCode = fat_set_fat_entry_value(sb_info, lFstCluLO, CLUSTER_NULL, TRUE, NULL);
        if(_ERR_FAT_SUCCESS != lErrCode)
        {
            D( ( DL_FATERROR, "in fat_trunc_fatentry,fat_set_fat_entry_value failed, ErrorCode = %d \n", lErrCode));
            return lErrCode;
...
delete_entry:
    ...
    if(!bRename && (de->DIR_FstClusLO != CLUSTER_FREE || de->DIR_FstClusHI != CLUSTER_FREE))
    {
        ...
        lErrCode = fat_trunc_fatentry(sb_info, FstCluster, TRUE);
        if(_ERR_FAT_SUCCESS != lErrCode)
        {
                       ...
                       UINT16 bsec, UINT16 eoff, UINT8 *ebuf, UINT8 *ecount)
{
    ...
    while(!fat_is_last_cluster(sb, clunum))
    {
        for(cursec = secnum; cursec < sb->iSecPerClus; cursec++)
        {
            ...
            //   DSM_MemCpy(g_fs_tst_buff,secbuf,512);
            for(secoff = eoff; secoff < DEFAULT_SECSIZE; secoff += DEFAULT_DIRENTRYSIZE)
            {
                UINT8 *np = (UINT8 *)(secbuf + secoff);
                switch(status)
