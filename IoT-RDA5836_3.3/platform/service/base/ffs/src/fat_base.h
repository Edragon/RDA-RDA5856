...
//serial number of long entry
#define FAT_LONG_ENTRY_NO(buff)     ((buff)[FLDE_LDIR_ORD_POS] & 0x3F)
...
typedef struct fat_inode_info
{
    ...
    UINT32 Dirty;                    // Entry's dirty flag.
...
typedef enum
{
    ...
    GET_LAST_LONG_ENTRY,
    GET_FULL_LONG_ENTRY,
}FAT_LONG_ENTRY_PARSE_STATUS;

typedef struct _FAT_LONG_ENTRY_PARSE_T
{
    FAT_LONG_ENTRY_PARSE_STATUS parseStatus;
    UINT32 lastEntryClu;
    UINT16 lastEntrySec;
    UINT16 lastEntryOff;
    UINT8 longEntryNum;
    UINT8 prevLongEntryNo;
    BOOL resetNextRound;
}FAT_LONG_ENTRY_PARSE_T;
...
#define FAT_OEMNAME                             "RDA5856"                        // oem name
...
INT32 fat_trunc_fatentry(FAT_SB_INFO *sb_info, UINT32 lFstCluLO, BOOL isRemove);
