...
INT32 fat_file_read(struct file * filp, INT8 * buf, INT32 len)
{
    ...
    if(fpos > fsize)
    {
        D( ( DL_FATERROR, "in fat_file_read. fpos(0X08%x) >  fsize(%d).\n", fpos, fsize) );
    ...
    iResult = fat_fpos2CSO(filp->f_inode, fpos, &clunum, &secnum, &secoff);
    ...
    while(!fat_is_last_cluster(sb, clunum))
    {
        ...
        // Get next cluster, secnum, secoff
        iResult = fat_fpos2CSO(filp->f_inode,(UINT32) filp->f_pos, &clunum, &secnum, &secoff);
        if (iResult != _ERR_FAT_SUCCESS)
        {
            // Out of file size,error.
...
INT32 fat_get_fat_entry_value(FAT_SB_INFO *sb_info, UINT32 fatent, UINT32 *value)
{
    ...
    else
    {
        ...
        *value &= 0x0FFFFFFF; // Found:FAT32的表项实际上只用了28位。
    }
...
INT32 fat_file_write(struct file * filp, INT8 * buf, UINT32 lenToWrite)
{
    ...
    {
        iRet = fat_fpos2CSO(pinode, (UINT32)filp->f_pos, &clunum, &secnum, &secoff);
...
INT32 fat_file_extend(struct file * filp, UINT64 SizeExtend)
{
    ...
    iRet = fat_fpos2CSO(pinode, (UINT32)pinode->i_size, &clunum, &secnum, &secoff);
    if (_ERR_FAT_SUCCESS == iRet)
    {
        DSM_ASSERT((BOOL)(clunum >= 2),"clunum = %d.",clunum);
...
INT32 fat_file_truncate(struct inode *psInode, UINT64 nFileSize)
{
    ...
    UINT32 lastCluster = 0, nBegCluToTrunc = 0;
    ...
    iRet = fat_fpos2CSO(psInode, (UINT32)nFileSize, &nClu, &nSec, &nOff);
    if (iRet != _ERR_FAT_SUCCESS)
    {
        g_TstFsErrCode = 1023;
        iRet = _ERR_FAT_ERROR;
    ...
    if (lastCluster != 0)
    {
        DSM_ASSERT((BOOL)(lastCluster >= 2),"lastCluster= %d.",lastCluster);
        iRet = fat_set_fat_entry_value(sb_info, lastCluster, CLUSTER_NULL, FALSE, NULL);
        if(_ERR_FAT_SUCCESS != iRet)
        {
            D( ( DL_FATERROR,"in fat_trunc_file,fat_set_fat_entry_value failed, ErrorCode = %d \n",iRet));
            goto end;
        }
    }

    iRet = fat_trunc_fatentry(sb_info, nBegCluToTrunc, FALSE);
    if(_ERR_FAT_SUCCESS != iRet)
    {
