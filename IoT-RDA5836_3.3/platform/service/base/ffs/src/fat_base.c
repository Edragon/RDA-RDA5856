...
static FAT_CACHE g_FatCache_Def[FS_DEFAULT_DEV_COUNT]; //we now only have two uFsDevCount
FAT_CACHE* g_FatCache = g_FatCache_Def;
#else
...
VOID SetVolLabEntry(PCSTR pVolLab, UINT8 *pBuff)
{
    ...
    DSM_MemSet(sFde.DIR_Name, 0, FAT_MAX_NAME_LEN);
...
//	iBegClus = iCurClus;
        ...
        FAT_LONG_ENTRY_PARSE_T longEntryParse;

        longEntryParse.parseStatus = NO_LONG_ENTRY;
        longEntryParse.resetNextRound = TRUE;
        longEntryParse.lastEntryClu = 0;
        longEntryParse.lastEntryOff = 0;
        longEntryParse.lastEntrySec = 0;
        longEntryParse.longEntryNum = 0;
        longEntryParse.prevLongEntryNo = 0;
	...
	{	
		...
		for(iCurSec = iCurSec; iCurSec < iMaxSecCount; iCurSec++)
		{
			...
			for(i = iCurEOff; i < DEFAULT_SECSIZE; i += DEFAULT_DIRENTRYSIZE)
			{
				...
				if(longEntryParse.parseStatus != NO_LONG_ENTRY)
				{
                                if(longEntryParse.resetNextRound)
                                {
                                    longEntryParse.parseStatus = NO_LONG_ENTRY;
                                }
                                else
				...
				else if(FAT_IS_SHORT_ENTRY(pSecBuf+i))	 // short entry  
				{
					...
					else
					{
						...
						{ 
                                                    if(GET_FULL_LONG_ENTRY == longEntryParse.parseStatus)
                                                    {
        							iCurEntryClus = longEntryParse.lastEntryClu;
                                                        iCurEntrySec = longEntryParse.lastEntrySec;
        							iCurEntryEOff = longEntryParse.lastEntryOff;
                                                        longEntryParse.parseStatus = NO_LONG_ENTRY;
                                                    }
                                                    else
                                                    {
						...
						if(mode == GET_PREV_ENTRY)
						{
							...
							{
								if(iLastClus == 0 && iLastSec == iStartSec && iLastEOff == 0 && (iFileNum <= 1))
									iRet = _ERR_FAT_FILE_NOT_EXISTED;
								else
								{
									iBegSec = iLastSec;
				...
				else if(FAT_IS_FIRST_LONG_ENTRY(pSecBuf+i) || FAT_IS_MIDDLE_LONG_ENTRY(pSecBuf+i) || FAT_IS_LAST_LONG_ENTRY(pSecBuf+i))
				{
					...
					if(FAT_NOT_SUPPORT_LONG_NAME == sb_info->iSuppLName)
					{
						continue;
                                ...
                                longEntryParse.resetNextRound = FALSE;
                                if(FAT_IS_LAST_LONG_ENTRY(pSecBuf+i))
                                {
                                    longEntryParse.parseStatus = GET_LAST_LONG_ENTRY;
                                    longEntryParse.lastEntryClu = iCurClus;
                                    longEntryParse.lastEntrySec = iCurSec;
                                    longEntryParse.lastEntryOff = i;
                                    longEntryParse.longEntryNum= FAT_LONG_ENTRY_NO(pSecBuf+i);
                                    longEntryParse.prevLongEntryNo = FAT_LONG_ENTRY_NO(pSecBuf+i);
                                }
                                else
                                {
                                    switch(longEntryParse.parseStatus)
                                    {
                                        case NO_LONG_ENTRY:
                                            break;
                                        case GET_LAST_LONG_ENTRY:
                                            if(FAT_LONG_ENTRY_NO(pSecBuf+i) + 1 == longEntryParse.prevLongEntryNo)
                                            {
                                                longEntryParse.prevLongEntryNo = FAT_LONG_ENTRY_NO(pSecBuf+i);
                                            }
                                            else
                                ...
                                if( (FAT_LONG_ENTRY_NO(pSecBuf+i) == 1) && (GET_LAST_LONG_ENTRY == longEntryParse.parseStatus) )
                                {
                                    longEntryParse.parseStatus = GET_FULL_LONG_ENTRY;
                                }
...
//
UINT32 fat_fpos2cluster(struct inode *inode, UINT32 iPos)
{
    FAT_SB_INFO* sb = NULL;
    UINT32 clus_num = 0;
    UINT32 clus_index;
    UINT32 cur_clus_index;
    UINT32 cur_clus_num;
    UINT32 nex_clus_num = 0;    
    INT32 result;

    sb = &(inode->i_sb->u.fat_sb);
    clus_index = iPos / (sb->iSecPerClus * DEFAULT_SECSIZE);
    cur_clus_index = inode->u.fat_i.ClusIndx;
    if(cur_clus_index <= clus_index)
    {
        cur_clus_num = inode->u.fat_i.CurClus;
    }
    else
    {
        cur_clus_num = ((inode->u.fat_i.entry.DIR_FstClusHI << 16) | inode->u.fat_i.entry.DIR_FstClusLO);
        cur_clus_index = 0;
    }
    while(IS_VALID_CLUSTER(sb,cur_clus_num))
    {
        if(cur_clus_index == clus_index)
        {
            clus_num = cur_clus_num;
            inode->u.fat_i.CurClus = cur_clus_num;
            inode->u.fat_i.ClusIndx = cur_clus_index;
            break;
        }
        result = fat_get_next_cluster(sb,cur_clus_num,&nex_clus_num);
        if(result == 0)
        {
            cur_clus_index ++;
            cur_clus_num = nex_clus_num;
        }
        else
        {
            clus_num = 0;
...
//
INT32 fat_fpos2CSO(struct inode *inode, UINT32 iPos, UINT32 *piClu, UINT32 *piSec, UINT32 *piOff)
{
    FAT_SB_INFO* sb = NULL;
    UINT8 secperclu = 0;
    ...
    *piClu = fat_fpos2cluster(inode,iPos); 
    if (0 == *piClu)
    {
...
INT32 fat_get_next_cluster(FAT_SB_INFO *sb_info, UINT32 iPreClu, UINT32 *pNexClu)
{
    ...
    if(sb_info->iFATn == FAT12)
    {
        if(ThisFATEntOffset == 0x1FF)
        {
            fat12LastByte = psFatCache->szSecBuf[ThisFATEntOffset];
            if ( psFatCache->iSecNum != (ThisFATSecNum+1) )
            {
                if (psFatCache->iSecNum != 0)
                {
                    iResult = fat_synch_fat_cache(sb_info, psFatCache);
                ...
                iResult = DRV_BLOCK_READ( sb_info->sb->s_dev, (ThisFATSecNum+1), psFatCache->szSecBuf);
                if(_ERR_FAT_SUCCESS != iResult)
                {
                    psFatCache->iSecNum = 0;
                    iRet = _ERR_FAT_READ_SEC_FAILED;
                ...
                psFatCache->iSecNum = (ThisFATSecNum+1);
                psFatCache->iDirty = 0;
            }
            FATCluEntry = MAKEFATUINT16(fat12LastByte, psFatCache->szSecBuf[0x0000]);
        }
        else
        {
                        ...
                        UINT32 *pLossy)
{
	...
	*pLossy = FAT_LOSSY_CONVERSION;
    ...
    for(i=0,j=0; pszLongName[i]!=0; i++)
    {
        if((j<8)&&(iDotCount<0))
        {
            DSM_HstSendEvent( pszLongName[i]);
            DSM_HstSendEvent( pszLongName[i]&0xff00);
            if((pszLongName[i]&0xff00) != 0x0000)  //if is unicode
            {
                DSM_HstSendEvent( 0x55555555);
                pszPName[j++] = (UINT8)(pszLongName[i]&0x00ff);
                if(j>=8)
                    continue;
                pszPName[j++] = (UINT8)((pszLongName[i]>>8)&0x00ff);
            }
            else
            {
                DSM_HstSendEvent( 0x55555551);
    ...
    pszPName[j] = 0;
    *piPLen = j;
    //app_trace(1, "i is %d, j is %d, pszPName is %s", i, j, pszPName);
    ...
    *piELen = j;        
    *pLossy = iLossy;  
    //app_trace(1, "i is %d, j is %d, pszEName is %s", i, j, pszEName);
                        ...
                        UINT32 *pLossy)
{
    ...
    for(i=0,j=0; pszLongName[i]!=0; i++)
    {
        if(j<8)
        {
            if((pszLongName[i]!='.') && (pszLongName[i]!='"')
                &&(pszLongName[i]!='/') && (pszLongName[i]!='\\')
                &&(pszLongName[i]!='[') && (pszLongName[i]!=']')
                &&(pszLongName[i]!=':') && (pszLongName[i]!=';')
                &&(pszLongName[i]!='=') && (pszLongName[i]!=',')
                &&(pszLongName[i]!=' '))
            {
                p = (UINT8*)(&pszLongName[i]);
                if((*p) != 0)
                {
                    pszShortName[j++]  = *p++;
                }
    ...
    if(j<8)
    {
        DSM_MemSet(pszShortName+j,' ',8-j);
    }

    // ignores the final period 
    if( (iDotCount + 1 = i) && (iPrevDotCount >= 0) )
    {
        iDotCount = iPrevDotCount;
    }

    if(iDotCount >= 0)
    {
        for(i=iDotCount+1,j=0; pszLongName[i]!=0; i++)
        {
            if(j<3)
            {
                if((pszLongName[i]>='A' && pszLongName[i]<='Z')
                    ||(pszLongName[i]>='0' && pszLongName[i]<='9'))
                pszShortName[8+(j++)] = pszLongName[i];
            }
        }
    }
    ...
    if(j<3)
    {
        DSM_MemSet(pszShortName+8+j,' ',8-j);
    }

    pszShortName[11] = 0;
    
    app_trace(1, "pszShortName is %s", pszShortName);
...
INT32 fat_get_short_name(FAT_SB_INFO* sb_info, TCHAR* filename, UINT32 bclu, UINT16 bsec, UINT16 eoff)
{
	...
	{
	    //hal_DbgAssert("aaa");
		D( ( DL_FATERROR, "In fat_get_short_name paramert error.bclu  == %d\n", bclu) );
		return _ERR_FAT_PARAM_ERROR;
	}
	...
	if(NULL == pSecBuf)
	{
		D( ( DL_FATERROR, "In fat_get_short_name malloc error \n") );
		DSM_ASSERT(0,"In fat_get_short_name malloc error \n");
		return _ERR_FAT_MALLOC_FAILED;		  
	}
	...
	else // FAT12/FAT16 root dir.
	{
		if(sb_info->iBytesPerSec == 0)
		{
			 D( ( DL_FATERROR, "in fat_get_short_name sb_info->iBytesPerSec = %d.\n", sb_info->iBytesPerSec)); 
			 if(NULL != pSecBuf)
			 {
				FAT_SECT_BUF_FREE((SECT_BUF*)pSecBuf);
			 }
	...
	{	
		...
		for(; iCurSec < iMaxSecCount; iCurSec++)
		{
			...
			if(_ERR_FAT_SUCCESS != iResult)
			{				 
				D( ( DL_FATERROR, "In fat_get_short_name DRV_BLOCK_READ error ErrorCode = %d \n",iResult) );
				iRet = _ERR_FAT_READ_SEC_FAILED;
			...
			for(i = iCurEOff; i < DEFAULT_SECSIZE; i += DEFAULT_DIRENTRYSIZE)
			{
				if(FAT_IS_SHORT_ENTRY(pSecBuf+i))	// short entry	
				{
					if(FAT_IS_JAP_DELETE_ENTRY(pSecBuf + i)) // Japonic delete entry continue
					{
						D( ( DL_FATBRIEF, "In fat_get_short_name FAT_IS_FREE_ENTRY : i = 0x%x\n",i) );
						iRet = _ERR_FAT_ERROR;
						goto fat_get_short_name;
					}
				...
				{
					D( ( DL_FATBRIEF, "In fat_get_short_name FAT_IS_INVALID_ENTRY : i = 0x%x\n", i) );
					iRet = _ERR_FAT_ERROR;
					goto fat_get_short_name;
				}
			}
		...
		if(FAT32 == sb_info->iFATn || 2 <= bclu) // FAT12/FAT16 sub dir or FAT32 dir.
	  {
		  ...
		  {
			   iRet = _ERR_FAT_FILE_NOT_EXISTED;
			  D((DL_FATERROR, "fat_get_short_name: Can't find the entry,next cluster = 0x%x.", iRet));
			  goto fat_get_short_name;
		  }
		  iCurSec = 0;
		  iCurEOff = 0;
	  ...
	  {
		  iRet = _ERR_FAT_FILE_NOT_EXISTED;
		  D((DL_FATERROR, "fat_get_short_name: can't find the entry."));
		goto fat_get_short_name;
	  } 
