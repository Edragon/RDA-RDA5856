...
INT32 fat_read_inode ( struct inode *inode)
{
    ...
    if(FS_ROOT_INO == inode->i_ino)
    {
        ...
        inode->u.fat_i.CurClus = fsbi->iFat32RootClus;
        inode->u.fat_i.ClusIndx = 0;
    ...
    else
    {
        ...
        if(bclu >= 2)
        {
            ret = fat_read_entries(fsbi, bclu, bsec, soff, ebuf, &ecount);
        }
        else if(bclu == 0)
        {
            ret = fat_read_root_entries(fsbi, bsec, soff, ebuf, &ecount);
        }
       else
        {
        ...
        inode->u.fat_i.CurClus = ((entry.DIR_FstClusHI << 16) | entry.DIR_FstClusLO);
	 inode->u.fat_i.ClusIndx = 0;
        inode->u.fat_i.entry = entry;
        inode->u.fat_i.Dirty = DATA_CLEAN;
    }
...
INT32 fat_trunc_file( struct inode *dir_i, struct dentry *de, int mode )
{
    ...
    // free fat_chain
    lErrCode = fat_trunc_fatentry(sb_info, firstCluster, FALSE);
    if(_ERR_FAT_SUCCESS != lErrCode)
    {
...
INT32 fat_scan_disk(struct super_block* sb)
{
    ...
    CSW_TRACE(DL_FATERROR, "in fat_scan_disk uDevNo = %d,iClusCount = 0x%x.",sb->s_dev,iClusCount);
