...
#if defined(_FS_SIMULATOR_) ||defined(_T_UPGRADE_PROGRAMMER)
#define FS_SemInit() 
#define FS_Down()    
#define FS_Up()   
#else
...
INT32 FS_GetRootType(FS_DEV_TYPE *pDevType, UINT8 *pFsType)
{
    ...
    *pFsType = g_fs_current.root->i_fstype;
    DSM_HstSendEvent((UINT32)(*pDevType));
    DSM_HstSendEvent((UINT32)(*pFsType));
    DSM_HstSendEvent(0xaabb1001);
    return ERR_SUCCESS;
}
...
INT32 FS_HasFormatted(PCSTR pszDevName, UINT8 iFsType)
{
    ...
    INT32 iResult;
    INT32 iRet;
    DSM_HstSendEvent(0xaabb2000);
    if ( !pszDevName  || FS_TYPE_FAT != iFsType)
    {
        DSM_HstSendEvent(0xaabb2001);
        return ERR_FS_INVALID_PARAMETER;
    }   
    CSW_TRACE(1,"FS_HasFormatted pszDevName =%s",pszDevName);
    ...
    DSM_HstSendEvent(0xaabb2004);
    DSM_HstSendEvent(1-iRet);
    return iRet;
}
...
INT32 FS_Format(PCSTR pszDevName, UINT8 iFsType, UINT8 iFlag)
{
    UINT32 uDevNo = 0;
    INT32 iRet;
    DSM_HstSendEvent(0xaabb3000);
    if (NULL == pszDevName
        || ((FS_TYPE_FAT != iFsType) && (FS_TYPE_EXFAT != iFsType)))
    {
        return ERR_FS_INVALID_PARAMETER;
    }
    ...
    if(_ERR_FS_SUCCESS == iResult)
    {
        switch(pSB->s_fstype)
        {
            case FS_TYPE_FAT: 
                ...
                psFSInfo->iTotalSize = (UINT64)((UINT64)(fat_sb_info->iDataSec)/(UINT64)(fat_sb_info->iSecPerClus))*(UINT64)iSizePerClus; 
...
INT32 FS_MountDevice(FS_DEV_TYPE dev_type)
{
    ...
    //DSM_HstSendEvent(0xaabbC000);
    //dev_type = FS_DEV_TYPE_TFLASH;
    DSM_MemSet(dev_name,0,sizeof(dev_name));
    if(dev_type == FS_DEV_TYPE_TFLASH)
    {
        DSM_HstSendEvent(0xaabbC001);
        DSM_StrCpy(dev_name,"mmc0");
    }
    else if(dev_type == FS_DEV_TYPE_USBDISK)
    {
        DSM_HstSendEvent(0xaabbC002);
        DSM_StrCpy(dev_name,"usb0");
    }
    else
    {
        DSM_HstSendEvent(0xaabbC003);
        return ERR_INVALID_PARAMETER;
    }

    iResult = FS_HasFormatted(dev_name, FS_TYPE_FAT);
    DSM_HstSendEvent(0xaabbC033);
    DSM_HstSendEvent(0 - iResult);
    DSM_HstSendEvent(0 - ERR_FS_HAS_FORMATED);
    if(iResult == ERR_FS_HAS_FORMATED)
    {
        for ( i = 0; i < 3; i++ )
        {
            iResult = FS_MountRoot(dev_name);
            DSM_HstSendEvent(0xaabbC004);
            DSM_HstSendEvent(0 -iResult);
            ...
            else if(iResult == ERR_FS_HAS_MOUNTED)
            {
                iRet = FS_UnmountRoot();
            }
        }
        ...
        /*if ( iResult == ERR_SUCCESS 
            ||iResult == ERR_FS_HAS_MOUNTED)
        {
            DSM_HstSendEvent(0xaabbC005);
            DSM_HstSendEvent(0 -iResult);
            iRet = ERR_SUCCESS;
        }*/
    }
    ...
    {
    	DSM_HstSendEvent(0xaabbC006);
    	DSM_HstSendEvent(0 -iResult);
       iRet = iResult;
    }
    DSM_HstSendEvent(0xaabbC007);
...
INT32 FS_MountRoot( PCSTR pszRootDevName )
{
    ...
    if(NULL == pszRootDevName)
    {
        ...
        if(iResult != ERR_SUCCESS)
        {
               DSM_ASSERT(0,"FS_MountRoot1:iResult = %d",iResult);
               return ERR_FS_DEVICE_NOT_REGISTER;
        }
    ...
    if(DSM_IsRemoveableDev(uDevNo))
    {
        ...
        else if(sb->s_fstype == FS_TYPE_EXFAT)
        {
            DRV_SET_WCACHE_SIZE(uDevNo,sb->u.exfat_sb.SectorsPerCluster*DEFAULT_SECSIZE);
            DRV_SET_RCACHE_SIZE(uDevNo,sb->u.exfat_sb.SectorsPerCluster*DEFAULT_SECSIZE*2);
        }
        else
        {
            DSM_ASSERT(0,"Unknow file system type:%d.",sb->s_fstype);
        }
        if(!fs_is_opened(uDevNo))
       {
          DRV_DEV_DEACTIVE(uDevNo);
...
{
    struct inode *mount_point_i = NULL, *inode = NULL;
...
INT32 FS_Mount( PCSTR pszDevName, PCSTR pszMountPoint, INT32 iFlag, UINT8 iType )
{
    ...
    root_i = iget( sb, (UINT64)FS_ROOT_INO);
...
step0_failed:
    ...
    FS_Up();
    DSM_HstSendEvent(0xaabb1101);
    DSM_HstSendEvent((UINT32)(-iRet));
    return iRet;
}
...
INT32 FS_ChangeDir( PCSTR pszDirName )
{
    ...
    UINT16 iULen = 0;
    UINT32 uDevNo = 0;
	char current_dir[ 255 + 1 ] = { '/', '>', };
    DSM_HstSendEvent(0xaab11000);
    ...
    FS_Down();
    /*namei()--begin*/
	if(g_fs_current.pwd->i_fstype == FS_TYPE_EXFAT && DSM_TcStrCmp(path,(PCWSTR)L".." ) == 0 )
	{
		...
		path = format_dir_name((PCWSTR)current_dir);
		if(!path)
		{
			D( ( DL_FATERROR, "in FS_ChangeDir, path is invalid" ) );
			return ERR_FS_NO_MORE_MEMORY;
    ...
    if(iRet != 0)
    {
        D( ( DL_FATERROR, "in FS_ChangeDir, find_entry error iRet = %d",iRet) );
        iRet = ERR_FS_NO_DIR_ENTRY;
        goto step1_failed;
    }
    ...
    iRet = fs_set_work_dir(g_fs_current.wdPath, (PCWSTR)pszDirName);
    if (iRet != ERR_SUCCESS)
    {
        D((DL_FATERROR, "fs_set_work_dir: fs_set_work_dir return %d", iRet));
        iput(inode);
        goto step1_failed;
    }
...
INT32 FS_Close(INT32 fd)
{
    ...
    FS_Down();
    iRet = fs_close(fd);
    if(ERR_SUCCESS == iRet && DSM_IsRemoveableDev(uDevNo))
    {
        iRet = DRV_DEV_FLUSH(uDevNo);
        if(ERR_SUCCESS != iRet)
        {
...
INT32 FS_Seek(INT32 fd, INT32 iOffset, UINT8 iOrigin)
{
 ...
 //   FS_Down();
    ...
    switch ( iOrigin )
    {
        case FS_SEEK_SET:
            tmp = 0 + (INT64)iOffset;
            file->f_pos = tmp < 0 ? 0 : (tmp > file->f_inode->i_size ? file->f_inode->i_size: tmp);
            iRet = (INT32)file->f_pos;
            break;
    ...
    CSW_TRACE(BASE_DSM_TS_ID, "EXFAT_DEBUG:FS_Seek() end,fd = 0x%x,iRet = 0x%x.",
            fd,iRet);
 //   FS_Up();
    return iRet;    
}
...
INT64 FS_Seek64(INT32 fd, INT64 iOffset, UINT8 iOrigin)
{
    struct file * file;
    INT64 tmp = 0;
    INT64 iRet = ERR_SUCCESS;
    DSM_HstSendEvent(0xaab17000);
    CSW_TRACE(BASE_DSM_TS_ID, "EXFAT_DEBUG:FS_Seek() begin,fd = 0x%x,iOffset = 0x%llx,iOrigin = 0x%x.",
            fd,iOffset,iOrigin);
     // DSM_ASSERT(fd >= 0);
    if ( !FD_IS_VALID( fd ) )
...
char *FS_GetLine(char *s, int size, INT32 f)
{
    int i, len;

    static UINT8 * secbuf = NULL;
    static INT32 buf_pos = 0;
    static UINT32 buf_size = 0;
    DSM_HstSendEvent(0xaab1B000);
    // reset buffer
    if(s==NULL && size ==0)
    {
        buf_pos = buf_size = 0;
    ...
    // no more data
    if(buf_pos>= buf_size && FS_IsEndOfFile(f))
    {
        if(secbuf != NULL)
        {
    ...
    if(buf_size-buf_pos < size)
    {
        if(buf_size > buf_pos)
  	 {
            DSM_MemCpy(secbuf, secbuf+buf_pos, buf_size-buf_pos);
      	 }
        buf_pos = buf_size-buf_pos;
        len = FS_Read(f, secbuf+buf_pos, DEFAULT_SECSIZE-buf_pos);
        buf_size = buf_pos + len;
        buf_pos = 0;
    }

    // get line from buffer
    for(i=0;buf_pos<buf_size;i++)
    {
        s[i] = secbuf[buf_pos++];
        if(s[i] == '\n')
        {
...
#endif
UINT32 FS_GetFileINo(INT32 hFile, UINT32 *iNO)
{
    struct file * psFile;
    INT32 iErrCode;
    DSM_HstSendEvent(0xaab21000);
    if( !FD_IS_VALID( hFile ) )
    {
        D( ( DL_FATERROR, "in FS_GetFileINo, invalid fd = %d\n",hFile) );
    ...
    if(iNO == NULL)
    {
        return ERR_NOTE_INVALID_PARAMETER;
    }
    ...
    {
        D( ( DL_FATERROR, "in FS_GetFileINo, psFile is NULL.\n") );
        iErrCode = ERR_FS_BAD_FD;
        FS_Up();
        return iErrCode;
    }
    *iNO = psFile->f_inode->i_ino;
    FS_Up();
    return ERR_SUCCESS;
}
...
INT32 FS_PowerOn(VOID)
{
    ...
    { 
       ...
       #else
       g_fs_sem = COS_CreateSemaphore(1);
       #endif
    }
...
INT32 FS_PowerOn(VOID)
{
    ...
    //INT32 iRet = ERR_SUCCESS;
    UINT32 i;
    DSM_HstSendEvent(0xaabb2220);
    if(g_pFsDevInfo != NULL) // FS_PowerOn already
        return ERR_SUCCESS;
