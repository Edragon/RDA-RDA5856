...
extern "C" {
#endif
#define A2DP_MAX_MEDIA_LEN                 500
#define SOUNDBAR_SYNC_DELAY_TIME           220
...
typedef struct soundbar_avdtp_state
{
    u_int8 avdtp_codec_type;
    u_int8 avdtp_protection;
    u_int8 avdtp_state;
}soundbar_avdtp_status;

APIDECL1 t_api APIDECL2 Soundbar_Set_Avdtp_status(soundbar_avdtp_status * set_status ,u_int8 index);
APIDECL1 t_api APIDECL2 Soundbar_Get_Avdtp_status(soundbar_avdtp_status * get_status ,u_int16 cid);
APIDECL1 t_api APIDECL2 Soundbar_Spp_Send_Data(u_int8 * data, u_int16 length);
APIDECL1 t_api APIDECL2 Soundbar_Get_Avdtp_Sbc_Header_Num(u_int8 index);
APIDECL1 t_api APIDECL2 Soundbar_Avdtp_Recieve_Data(t_DataBuf * p_buf,u_int8 index);
void rdabt_a2dp_receive_spp_data(t_DataBuf * p_buf);

void Soundbar_Set_config(u_int16 sppport,u_int8 soundbarmaster);
u_int8 Soundbar_Get_Role(void);

u_int32 Soundbar_Sync_Play_Number(u_int8 flag);
void rdabt_a2dp_set_sourceclk(u_int8 role ,u_int8 cleanclk);  
u_int32 rdabt_a2dp_get_sourceclk(void);
void rdabt_a2dp_set_syncclk(u_int32 clk);
u_int32 rdabt_a2dp_get_syncclk(void);
u_int32 rdabt_a2dp_check_syncclock(u_int32 milliseconds);
void Soundbar_Set_UnSniff_Mode(u_int8 newmode);

typedef struct {
    u_int32 clock;
    u_int8 sequencenum[2];
}soundbar_sequence_clock;
...
typedef struct{
    ...
    u_int32 cmdId;
    u_int32 cmd;
    u_int8 mediadata[A2DP_MAX_MEDIA_LEN];
}SOUNDBAR_MEDIA_DATA_STRUCT;
...
typedef struct{
    u_int32 cmdId;
    u_int32 cmd;
}SOUNBAR_CONTROL_CMD_STRUCT;

void Soundbar_Set_Control_Cmd(u_int32 cmdId,u_int32 cmd);
void Soundbar_Set_Audio_Sync(u_int32 synclk,u_int32 counternum);
APIDECL1 t_api APIDECL2 Soundbar_Set_Remote_SequenceClock_And_Check(u_int8 *data);
APIDECL1 t_api APIDECL2 Soundbar_Get_Delay_Times(void);
APIDECL1 t_api APIDECL2 Soundbar_Get_Error_Delay_Times(void);
APIDECL1 t_api APIDECL2 Soundbar_Get_Lostsequence_Times(void);
APIDECL1 t_api APIDECL2 Soundbar_Avdtp_Master_Stop_And_CleanStatus(void);
APIDECL1 t_api APIDECL2 Soundbar_Avdtp_Slave_Stop_And_CleanStatus(void);
...
APIDECL1 t_api APIDECL2 AVDTP_Disconnect(u_int16 cid, t_bdaddr addr);
...
APIDECL1 t_api APIDECL2 Avdtp_Send_Start(u_int16 cid, t_bdaddr addr);
APIDECL1 t_api APIDECL2 Avdtp_Send_Suspend(u_int16 cid, t_bdaddr addr);
...
#ifdef WIN32
void rdabt_a2dp_free_buf(struct st_t_dataBuf *p_buf, u_int8 type);
#endif
