...
UINT8 fp_checkflashpages[32][64][4];
...
PUBLIC BOOL fp_CheckFlashScratch(VOID)
{
    ...
    MEMD_ERR_T err;
	UINT32 timeSpend = hal_TimGetUpTime();
    fp_checkflash_flag = 0;
	if(MEMD_ERR_NO != memd_FlashErase((UINT8 *)flash_addr,(UINT8 *)flash_addr+0x2000))
    {
	...
	if(MEMD_ERR_NO != memd_FlashErase((UINT8 *)flash_addr,(UINT8 *)flash_addr+0x2000))
    {
        fp_checkflash_flag = 0x15092504;
        mon_Event(0x15092504);
        mon_Event(phy_addr);
        return FALSE;
    }
    //Prepare pattern data
	...
	for(flash_addr = 0x000000; flash_addr < 0x80000; flash_addr += 0x80000)
    {
        ...
        if(MEMD_ERR_NO != memd_FlashWrite((UINT8 *) flash_addr, sizeof(fp_checkflashpages), &WrittenByteSize,
                                       (UINT8 *) fp_checkflashpages))
        {
        	fp_checkflash_flag = 0x15092505;
            mon_Event(0x15092505);
            mon_Event(phy_addr);
            return FALSE;
        }
        ...
        if(0 != memcmp(phy_addr, fp_checkflashpages, sizeof(fp_checkflashpages)))
        {
        	fp_checkflash_flag = 0x15092506;
            mon_Event(0x15092506);
            mon_Event(phy_addr);
            return FALSE;
        }
    }
    ...
    if(MEMD_ERR_NO != memd_FlashErase((UINT8 *)flash_addr,(UINT8 *)flash_addr+0x2000))
    {
       fp_checkflash_flag = 0x15092507;
       mon_Event(0x15092507);
       mon_Event(phy_addr);
       return FALSE;
    }
    ...
    do{
          ...
          {
          	 fp_checkflash_flag = 0x15092508;
             mon_Event(0x15092508);
             mon_Event(phy_addr);
             return FALSE;
          }
    }while(++phy_addr < phy_addr_end);      
    fp_checkflash_flag = 0x1509250f;
	timeSpend = hal_TimGetUpTime()-timeSpend;
	mon_Event(timeSpend);
