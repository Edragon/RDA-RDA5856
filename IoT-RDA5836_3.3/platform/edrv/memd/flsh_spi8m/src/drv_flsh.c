...
UINT32 g_memdFlashBaseAddress = 0x01200000;
#else
UINT32 g_memdFlashBaseAddress = 0x08000000;
#endif
...
PUBLIC BOOL spi_flash_chip_erase(UINT32 flash_addr)
{
    ...
    addr_tmp = flash_addr&0xffffff;
    hal_SpiFlashSendCmd(OPCODE_WRITE_ENABLE, addr_tmp, opmode, 0);
    hal_SpiFlashSendCmd(OPCODE_CHIP_ERASE,addr_tmp,opmode,0);
...
{
    HAL_SYS_SPIFLASH_FREQ_T flash_freq = MEMD_SPIFLASH_FREQ;
    ...
    flash_freq = HAL_SYS_SPIFLASH_FREQ_52M;

    p_flsh_config.clkDiv = 0x1;

    if(flash_freq > HAL_SYS_SPIFLASH_FREQ_52M)
        p_flsh_config.sampleDelay = 0x2;
    else
...
#ifdef SPI_FLASH_READ_DUAL_MODE
    //set twice frequency than quard mode
    p_flsh_config.clkDiv = 0x1;
#endif

    hal_SysSetSpiFlashClock(flash_freq);
...
#ifdef SPI_FLASH_READ_DUAL_MODE
    p_flsh_config.readMode = HAL_SPI_FLASH_READ_DUAL_MODE;
    spi_flash_quard_ctrl(FALSE,0);
#else
...
UINT8 fp_checkflashpages[2][8][32];
UINT32 fp_checkflash_flag = 0;
PUBLIC BOOL fp_CheckFlashScratch(VOID)
{
    UINT32 page, i;
    UINT32 flash_addr;
    UINT32 *phy_addr;
    ...
    fp_checkflash_flag = 0;

    //Prepare pattern data
    for(page = 0; page < sizeof(fp_checkflashpages) / sizeof(fp_checkflashpages[0]); page++)
    {
        for(i = 0; i < sizeof(fp_checkflashpages[0]) / sizeof(fp_checkflashpages[0][0]); i++)
        {
            if((i%2) == 0)
                memset(fp_checkflashpages[page][i], 0x55, sizeof(fp_checkflashpages[page][0]));
            else
                memset(fp_checkflashpages[page][i], 0xAA, sizeof(fp_checkflashpages[page][0]));
        }
    }


    for(flash_addr = 0x000000; flash_addr <= 0x80000; flash_addr += 0x80000)
    {
        phy_addr = (UINT32 *) (0xa8000000 | flash_addr);
        
        //Write pattern data flash
        if(MEMD_ERR_NO != memd_FlashWrite((UINT8 *) flash_addr, sizeof(fp_checkflashpages), &WrittenByteSize,
                                       (UINT8 *) fp_checkflashpages))
        {
        	fp_checkflash_flag = 0x15092500;
            mon_Event(0x15092500);
            mon_Event(phy_addr);
            return FALSE;
        }

        //Read back and compare with pattern data
        if(0 != memcmp(phy_addr, fp_checkflashpages, sizeof(fp_checkflashpages)))
        {
        	fp_checkflash_flag = 0x15092501;
            mon_Event(0x15092501);
            mon_Event(phy_addr);
            return FALSE;
        }
/*
        //Erase to 0XFFFFFFFF
        if(MEMD_ERR_NO != memd_FlashErase((UINT8 *)flash_addr,NULL))
        {
            mon_Event(0x15092502);
            mon_Event(phy_addr);
            return FALSE;
        }

        //Read back and compare with 0XFFFFFFFF
        phy_addr_end = (UINT32 *) (((UINT32) phy_addr) + sizeof(fp_checkflashpages));
        do{
                if(0XFFFFFFFF != *phy_addr)
                {
                    mon_Event(0x15092503);
                    mon_Event(phy_addr);
                    return FALSE;
                }
           }while(++phy_addr < phy_addr_end);
*/
    }

    fp_checkflash_flag = 0x1509250f;
    mon_Event(0x1509250f);
    mon_Event(&fp_checkflash_flag);

    return TRUE;    
}


#if 0// flash life test ramrum
#include "hal_dma.h"
#include "flash_prog_map.h"
#include "hal_gpio.h"

extern FLASH_PROG_MAP_ACCESS_T g_flashProgDescr;
PUBLIC INT32 fp_FlashLifeTest(UINT32 *cycle_count, UINT32 **error_address)
{
    INT32                                i = 0;
	...
	HAL_APO_ID_T gpioled;
	gpioled.gpioId = HAL_GPIO_0_4;
    gpio_KeyLedMode();

    memset(g_flashProgDescr.dataBufferA, 0x55, g_flashProgDescr.fpcSize);
    memset(g_flashProgDescr.dataBufferB, 0xaa, g_flashProgDescr.fpcSize);
    cmds[0].ramAddr = g_flashProgDescr.dataBufferA;
    cmds[0].fcs = 0x00490696;
    cmds[0].size = *((UINT32 *)g_flashProgDescr.dataBufferA);
    cmds[1].ramAddr = g_flashProgDescr.dataBufferB;
    cmds[1].fcs = 0x00e4834b;
    cmds[1].size = *((UINT32 *)g_flashProgDescr.dataBufferB);
    i = 0;
    ...
    while(1)
    {
        ...
        UINT32 flash_addr ;
        // earse
        for(flash_addr=0;flash_addr<g_memdFlashLayout.fullSize;flash_addr+=0x10000)
        {
            //mon_Event(memd_FlashGetGlobalAddress(flash_addr));
            verifySectorAddress = (UINT32*)HAL_SYS_GET_UNCACHED_ADDR(memd_FlashGetGlobalAddress(flash_addr));
            status = spi_flash_block_erase(memd_FlashGetGlobalAddress(flash_addr));
            if(!status)
            {
                status = FPC_ERASE_SECTOR;
                break;
            }
            else
                status = MEMD_ERR_NO;
            //mon_Event(verifySectorAddress);
            fcsCtx.srcAddr      = verifySectorAddress;
            fcsCtx.transferSize = 0x10000;
            ...
            if(0x00b262d2 != FCS)
            {
                for(cpt=0;cpt<0x10000;cpt+=4,verifySectorAddress++)
                {
                    if(*verifySectorAddress!=0xFFFFFFFF)
                    {
                        mon_Event(0xeeffeeff);
                        mon_Event(verifySectorAddress);
                        status = FPC_FLASH_NOT_AT_FF; // FCS error
                        break;
                    }
                }
            }
        }
        mon_Event(0xeeee0000 + (status != MEMD_ERR_NO));
        if(status != MEMD_ERR_NO)
        {
            mon_Event(status);
            break;
        ...
        for(flash_addr=0;flash_addr<g_memdFlashLayout.fullSize;flash_addr+=g_flashProgDescr.fpcSize)
        {
            //mon_Event(memd_FlashGetGlobalAddress(flash_addr));
            verifySectorAddress = (UINT32*)HAL_SYS_GET_UNCACHED_ADDR(memd_FlashGetGlobalAddress(flash_addr));
            status = memd_FlashWrite(flash_addr,
                                   g_flashProgDescr.fpcSize, &s,
                                   (CHAR*) (cmds[i].ramAddr));
            if(status != MEMD_ERR_NO)
            {
                status = FPC_PROGRAM;
            ...
            //mon_Event(verifySectorAddress);
            fcsCtx.srcAddr      = verifySectorAddress;
            fcsCtx.transferSize = g_flashProgDescr.fpcSize;
            fcsCtx.userHandler  = NULL;
            ...
            if(cmds[i].fcs != FCS)
            {
                for(cpt=0;cpt<g_flashProgDescr.fpcSize;cpt+=4,verifySectorAddress++)
                {
                    if(*verifySectorAddress!=cmds[i].size)
                    {
                        mon_Event(0xee55eeaa);
                        mon_Event(verifySectorAddress);
                        status = FPC_FCS_ERROR; // FCS error
                        break;
                    }
                }
            }
        ...
        mon_Event(0x55aa0000 + (status != MEMD_ERR_NO));
        if(status != MEMD_ERR_NO)
        {
            mon_Event(status);
            break;
        ...
        *cycle_count = *cycle_count + 1;
        mon_Event(0xef000000 + *cycle_count);
        i = !i;
        if(i)
            hal_GpioSet(gpioled);
        else
    ...
    *error_address = verifySectorAddress;
    return status;

}
#endif
#if (FLASH_LIFE_TEST_FOR_PUYA_8M_FLASH  == 1)  // flash life test ramrum for puya 8M flash test
#include "hal_dma.h"
#include "flash_prog_map.h"
#include "hal_gpio.h"
...
PUBLIC INT32 fp_FlashLifeTest(UINT32 *cycle_count, UINT32 **error_address)
{
    ...
    for(j = 0;j < 512;j++)
    {
		...
		*(g_flashProgDescr.dataBufferA+j) = 0xAA;
    	}
	else
	{
		*(g_flashProgDescr.dataBufferA+j) = 0x55;
	}
    }
    for(j =1;j < g_flashProgDescr.fpcSize/512;j++)
	memcpy((UINT8 *)g_flashProgDescr.dataBufferA+j*512,g_flashProgDescr.dataBufferA,512);
    ...
    for(j = 0;j < 512;j++)
    {
		...
		*(g_flashProgDescr.dataBufferB+j) = 0x55;
    	}
	else
	{
		*(g_flashProgDescr.dataBufferB+j) = 0xAA;
	}
    }
    for(j =1;j < g_flashProgDescr.fpcSize/512;j++)
	memcpy((UINT8 *)g_flashProgDescr.dataBufferB+j*512,g_flashProgDescr.dataBufferB,512);
    ...
    while(1)
    {
        ...
        UINT32 flash_addr ;
	//chip erase 
	setup_dual_page_mode();
	spi_flash_chip_erase(memd_FlashGetGlobalAddress(0x0));
        enter_test_mode_command();
	//dual page program 
	 for(flash_addr=0;flash_addr<g_memdFlashLayout.fullSize;flash_addr+=g_flashProgDescr.fpcSize)
        {
	   	for(j= 0;j < g_flashProgDescr.fpcSize;j+=512)
	   	{
	   		status = page_chip_program(flash_addr,(CHAR*) (cmds[i].ramAddr),512);
	   	}
           	 if(status != MEMD_ERR_NO)
            	{
               		 status = FPC_PROGRAM;
	 ...
	 for(flash_addr=0;flash_addr<g_memdFlashLayout.fullSize;flash_addr+=g_flashProgDescr.fpcSize)
	 {
		...
		if(cmds[i].fcs != FCS)
            	{
                	for(cpt=0;cpt<g_flashProgDescr.fpcSize;cpt+=512)
                	{
                    		for(j = 0;j < 512;j +=4,verifySectorAddress++)
                    		{
                    			if(*verifySectorAddress != *(UINT32 *)(cmds[i].ramAddr+j))
                    			{
                    				mon_Event(0xee55eeaa);
                        			mon_Event(verifySectorAddress);
                        			status = FPC_FCS_ERROR; // FCS error
