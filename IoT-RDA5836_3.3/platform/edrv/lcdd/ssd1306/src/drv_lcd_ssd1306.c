...
#else //SSD1306_USE_GOUDA
// LCDD screen config.
//static const LCDD_HAL_CONFIG_T g_tgtLcddCfg = TGT_LCDD_CONFIG;
...
PRIVATE VOID lcdSsd1306pWriteCmd(UINT8 cmd)
{
	...
	UINT32 data = 0x100;
        data |= cmd&0xff;
	hal_SpiSendData_32Bit(HAL_SPI_2,HAL_SPI_CS0,&data,1);
...
PRIVATE LCDD_ERR_T lcdSsd1306pOpen(VOID)
{
    hal_HstSendEvent(BOOT_EVENT, 0x01110010);
    lcdSsd1306pOpenSpi();
    lcdSsd1306pGpioInit();
    lcdSsd1306pReset();
...
{
   static char ssd1306_id_str[] = "ssd1306\n"; 
   return ssd1306_id_str; 
}
