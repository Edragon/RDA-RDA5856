                          ...
                          FMD_INIT_T* initValues)
{
    ...
    PREP_WRITE_REG_DATA(0x8000);
    fmd_Write(0x13, writeReg, 2);

	//PREP_WRITE_REG_DATA(0x5018);
    //fmd_Write(0x03, writeReg, 2);
...
PUBLIC FMD_ERR_T fmd_Tune(UINT32 freqKHz)
{
	...
	FMD_TRACE(EDRV_FMD_TRC,0,"writeReg: writeReg[0]=%02x,writeReg[1]=%02x",writeReg[0], writeReg[1]);
...
PUBLIC FMD_ERR_T fmd_Close(VOID)
{
   ...
   // rfd_XcvRegPuXtalEnable(FALSE);
    ...
    if (errStatus != FMD_ERR_NO)
    {
        FMD_TRACE(EDRV_FMD_TRC,0,"fmd_Close==%d",errStatus);
...
PUBLIC BOOL fmd_RegWrite(UINT32 addr, UINT32 data)
{
    UINT8 writeReg[2];
    PREP_WRITE_REG_DATA(data&0xffff);
    if (FMD_ERR_NO != fmd_Write(addr, writeReg,2))
    {
        return FALSE;
    }
...
PUBLIC BOOL fmd_RegRead(UINT32 addr, UINT32* pData)
{
    UINT8 readReg[2];
    
    if (FMD_ERR_NO != fmd_Read(addr, readReg, 2))
    {
        return FALSE;
    }
