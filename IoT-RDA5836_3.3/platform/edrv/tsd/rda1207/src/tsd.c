...
// ============================================================================
PRIVATE INT32 tsd_GpioGet(VOID)
{//all key status
    UINT32 regVal = 0;
    UINT32 counter = 0;
...
PRIVATE VOID tsd_KeyCallback_tmp(UINT32 x,TSD_PEN_STATE_T status)
{
    if (g_userKeyCallback == NULL)
    {
        return;
...
PRIVATE VOID tsd_Debounce(VOID)
{
    ...
    INT32 currentPenIrqPinState = 0;
    
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_8809P)
    currentPenIrqPinState = hal_GpioGet(g_tsdConfig->penGpio);
#elif (CHIP_ASIC_ID == CHIP_ASIC_ID_5855)|| (CHIP_ASIC_ID == CHIP_ASIC_ID_5856)
    currentPenIrqPinState = tsd_GpioGet();
#endif

#if (CHIP_ASIC_ID == CHIP_ASIC_ID_5855)|| (CHIP_ASIC_ID == CHIP_ASIC_ID_5856)
    if(currentPenIrqPinState < 0)// new in 5855. Sometimes we got -1 from gpio, for spi
    {
        EDRV_TRACE(EDRV_TSD_TRC, 0, "tsd_Debounce: Get wrong gpio status");
