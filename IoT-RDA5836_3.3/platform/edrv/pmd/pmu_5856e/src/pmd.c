...
volatile uint8 tsd_PowerOff =0x55;
#endif

//extern PUBLIC CONST TSD_CONFIG_T* tgt_GetTsdConfig(VOID);
extern PROTECTED UINT16 tsd_GetButtonIndex(UINT16 gpadcVal);
// =============================================================================
//  DEFINES
// =============================================================================
...
typedef VOID (*PMD_KEY_CALLBACK_T)(UINT8 button,TSD_EOM_STATE_T penState);
//PRIVATE PMD_KEY_CALLBACK_T g_KeyCallback = NULL;
//
// =============================================================================
//  TYPES
// =============================================================================
...
#endif
#if (0)//LCD_BACKLIGHT_CHARGE_PUMP)
...
PUBLIC VOID PMD_FUNC_INTERNAL pmd_OpalSpiWrite(RDA_REG_MAP_T regIdx, UINT16 value)
{
    ...
    //debug printf
	//hal_HstSendEvent(0xff, 0xFF000000 | (regIdx << 16) | value);
...
//	hal_HstSendEvent(0xff, 0x14041002);
//	hal_TimDelay(16);
...
PUBLIC VOID pmd_RegIrqSettingClr(UINT32 flags)
{
	...
	//hal_HstSendEvent(0xff, 0x14041003);
	//hal_HstSendEvent(0xff, flags);
...
// pmd_SwitchLdoDCDC
// ----------------------------------------------------------------------------
/// switch between Ldo and DCDC 
///
/// @param LdoDCDC(TRUE:Ldo; FALSE:DCDC) 
// ============================================================================
PROTECTED VOID pmd_SetDcdcLdoMode(BOOL LdoDCDC)
{
    UINT32 scStatus = hal_SysEnterCriticalSection();
    hwp_pmuc->pm_fsm_frc_ctrl1 &= ~PMUC_PU_BUCK_LDO_DR;
    
    if(LdoDCDC)
        hwp_pmuc->pm_fsm_frc_ctrl1 |= PMUC_PU_BUCK_LDO_REG;
    else
        hwp_pmuc->pm_fsm_frc_ctrl1 &= ~PMUC_PU_BUCK_LDO_REG;
...
PUBLIC VOID pmd_Open(CONST PMD_CONFIG_T* pmdConfig)
{
    ...
    hwp_pmuc->buck_cp_ctrl = 0x21691d;
#ifndef __CT_CHIP_PKG_RDA5856ETE__
    pmd_SetDcdcLdoMode(false);  //DCDC
#else
    pmd_SetDcdcLdoMode(true);   //LDO
#endif
    hwp_pmuc->ldo_cfg = 0x5505c;//0x4d05C;
    reg = hwp_pmuc->ldo_ctrl;
    reg &= ~PMUC_LDO_ULP_VRTC_VBIT_MASK;
    reg |= PMUC_LDO_ULP_VRTC_VBIT(2);
    hwp_pmuc->ldo_ctrl = reg;
#if (APP_SUPPORT_POWERKEY ==1)
    hwp_pmuc->pwr_sys_ctrl &=  ~PMUC_PM_REG_MASK;
    //enable power key
    hwp_pmuc->pmu_ana_rsvd = 0xf;
    hwp_pmuc->switch_mode = 1;
#endif
   ...
   hwp_pmuc->pm_fsm_frc_ctrl1 |=  (PMUC_MEM_GRP1_ON|PMUC_MEM_GRP2_ON |PMUC_MEM_GRP3_ON);
    ...
    //set 1 second interval for key power on
    hwp_pmuc->pwr_sys_ctrl |= PMUC_KEY_THLD(g_pmdConfig->keyPowerOnInterval);
...
//    UINT16 reg;
    ...
    //lower leakage at power down mode
    hwp_pmuc->ldo_ctrl &= ~PMUC_LDO_ULP_VRTC_VBIT_MASK;
...
PUBLIC VOID PMD_FUNC_INTERNAL pmd_SetPowerMode(PMD_POWERMODE_T powerMode)
{
    ...
    UINT32 scStatus = hal_SysEnterCriticalSection();
    hwp_pmuc->pwr_sys_ctrl &=  ~PMUC_PM_REG_MASK;
    hwp_pmuc->pwr_sys_ctrl |= PMUC_PM_REG(powerMode);
    hal_SysExitCriticalSection(scStatus);
}
...
#ifdef __CT_CHIP_PKG_RDA5856T__
...
PRIVATE VOID pmd_EnableChargePump(BOOL enable)
{
    ...
    hal_AudForceDcdc4ClockOn(enable);//edit later
...
PUBLIC VOID pmd_SoftStart()
{
	hal_HstSendEvent(SYS_EVENT, 0x16042701);
	UINT32 key_Data,pData,i =0;
	...
	hwp_pmu_intf->pmu_intf_imr |= PMU_INTF_KEY_MEASURE_INT_MASK|PMU_INTF_KEY_INT_MASK;
        hwp_sys_ctrl->per_module_en |= SYS_CTRL_GPADC_EN_ENABLE;

	pData = hwp_pmu_intf->pmu_intf_irsr;
	hwp_pmu_intf->pmu_intf_icr |= PMU_INTF_KEY_MEASURE_INT_CLEAR;
	while(!(pData&PMU_INTF_KEY_MEASURE_INT_RAW_STATUS))
	{
		pData = hwp_pmu_intf->pmu_intf_irsr;
		hwp_pmu_intf->pmu_intf_icr |= PMU_INTF_KEY_MEASURE_INT_CLEAR;
		if(i++>10)
	...
	i = 0;
	hal_HstSendEvent(SYS_EVENT, 0x16042707);
	hal_HstSendEvent(SYS_EVENT, pData);
	pmd_poweronKeyValue = 0xFFFFFFFF;
	if(pData&PMU_INTF_KEY_MEASURE_INT_RAW_STATUS)
	{
...
PUBLIC BOOL tsd_KeyOnOffStateAtPowerOn(VOID)
{
    if(0xFFFFFFFF != pmd_poweronKeyValue)
    {
    	...
    	//TSD_CONFIG_T* tsd_cfg_ptr = tgt_GetTsdConfig();
    	key_index = tsd_GetButtonIndex(pmd_poweronKeyValue&0x3ff);
    	hal_HstSendEvent(BOOT_EVENT, 0x16042702);
        hal_HstSendEvent(BOOT_EVENT, key_index);
        //Skip gpio key
...
#if (APP_SUPPORT_POWERKEY ==1)
PUBLIC void tsd_PowerKey(VOID)
{
    COS_EVENT ev;
	
    ev.nEventId  = EV_DM_POWER_KEY;
    ev.nParam1  = 0;
    
    hwp_pmu_intf->pmu_intf_imr |=PMU_INTF_SWITCH_INT_MASK;
...
PUBLIC BOOL tsd_KeyIsEnterDownloadMode(VOID)
{
    if(0xFFFFFFFF != pmd_poweronKeyValue)
    {
    	UINT16 key_index;
        ...
        //Skip gpio key
        key_index += tgt_GetTscKeyIndex();  
        return (KEY_PREV == tgt_GetKeyCode(key_index));
    }
...
PUBLIC VOID pmd_IrqHandler(UINT8 interruptId)
{
    ...
    // which is connected to GPIO_0 at baseband side.)
...
#if (APP_SUPPORT_POWERKEY ==1)
     if(hwp_pmu_intf->pmu_intf_isr & PMU_INTF_SWITCH_INT_STATUS) 
     {
	     hwp_pmu_intf->pmu_intf_imr |=PMU_INTF_SWITCH_INT_MASK;
     ...
     if(hwp_pmu_intf->pmu_intf_isr & PMU_INTF_AC_ON_INT_STATUS) 
     {
        hal_HstSendEvent(SYS_EVENT, 0x14041045);
        ...
        {
            g_pmdChargerInIrqEnabled = FALSE;
            pmd_DcOnHandler((hwp_pmuc->ana_status & PMUC_CHR_AC_ON) ? TRUE: FALSE);
        }
     }

     if(hwp_pmu_intf->pmu_intf_isr & (PMU_INTF_KEY_INT_STATUS | PMU_INTF_KEY_MEASURE_INT_STATUS))
     {
        ...
        hal_HstSendEvent(SYS_EVENT, 0x1404104a);
     }
...
PRIVATE VOID pmd_StartHpOutDetectInternal(VOID)
{
	...
	hal_HstSendEvent(0xff, 0x14041005);
...
PUBLIC VOID pmd_GpadcSetInterval(UINT32 interval)
{
    ...
    hwp_pmu_intf->gpadc_ctrl1 &= ~PMU_INTF_GPADC_START_TIME_SEL_MASK;
    hwp_pmu_intf->gpadc_ctrl1 |= PMU_INTF_GPADC_START_TIME_SEL(interval);
...
PUBLIC VOID pmd_GpadcEnableChannel(UINT32 channel)
{
    UINT32 scStatus;
    scStatus = hal_SysEnterCriticalSection();
    hwp_pmu_intf->gpadc_ctrl1 &= ~PMU_INTF_GPADC_CHANNEL_EN_DR;
    hwp_pmu_intf->gpadc_ctrl1 &= ~PMU_INTF_GPADC_CHANNEL_EN_REG_MASK;
    hwp_pmu_intf->gpadc_ctrl1 |= PMU_INTF_GPADC_CHANNEL_EN_REG(1<<channel)|PMU_INTF_GPADC_CHANNEL_EN_DR;
...
{
    UINT32 reg;
    reg = hwp_pmu_intf->gpadc_ctrl1;

    if( ((reg&PMU_INTF_GPADC_CHANNEL_EN_REG_MASK)==PMU_INTF_GPADC_CHANNEL_EN_REG(1<<channel)) &&
        (reg&PMU_INTF_GPADC_ENABLE) )
    {
        return TRUE;
    }
