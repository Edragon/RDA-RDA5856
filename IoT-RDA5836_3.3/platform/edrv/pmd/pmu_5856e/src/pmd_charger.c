...
#define PMD_Li_BATTERY_MAX_VOLTAGE (4350)
...
#if (PMD_MV_OFFSET_CHG_FULL_COMPENSATION >= PMD_MV_OFFSET_CHG_PHASE_3) ||  (PMD_MV_OFFSET_CHG_PHASE_3 >= PMD_MV_OFFSET_CHG_PHASE_2)
...
typedef enum
{
    //Audio Diff Signal(L channel) DC Cal.
    PMD_EFFUSE_PAGE_AUDIO_DIFF_L = 1,
    //Charger
    PMD_EFFUSE_PAGE_CHARGER = 3,
    //gpadc cal. 4.2V adc value(Bit9~2)
    PMD_EFFUSE_PAGE_VBAT_4P2 = 5,
    //gpadc cal. 2.7V adc value(Bit9~2)
    PMD_EFFUSE_PAGE_VBAT_2P7 = 7,
    //Audio Diff Signal(R channel) DC Cal.
    PMD_EFFUSE_PAGE_AUDIO_DIFF_R = 9,
    //keysense Cal. 10k adc value(Bit9~2)
    PMD_EFFUSE_PAGE_KEYSENSE_0K = 11,
    PMD_EFFUSE_PAGE_KEYSENSE_OHM_10K = 13,
    PMD_EFFUSE_PAGE_KEYSENSE_OHM_10K2 = 14,
    //keysense Cal. 68k adc value(Bit7~0)
    PMD_EFFUSE_PAGE_KEYSENSE_68K = 15,
} PMD_EFFUSE_PAGE_T;
...
PRIVATE UINT16 g_pmdChargerSetting2 = 
    ...
    RDA_PMU_EFUSE_CHR_VFB_SEL_REG(8);
...
PUBLIC UINT16 pmd_GetGpadcChargerLevel(VOID)
{
    ...
    hwp_pmuc->charger_cfg_ctrl &= ~PMUC_CHR_AUXADC_SEL_MASK;
    hwp_pmuc->charger_cfg_ctrl |= PMUC_CHR_AUXADC_SEL(2);
    readBat = hal_AnaGpadcGet(g_pmdConfig->chargerGpadcChannel);
    hwp_pmuc->charger_cfg_ctrl &= ~PMUC_CHR_AUXADC_SEL_MASK;
...
#if 1//def PMD_CHR_FULL_CHR_BY_CURRENT
PUBLIC UINT16 pmd_GetGpadcChargerCurrent(VOID)
{
    UINT16 readBat;
    
    hwp_pmuc->charger_cfg_ctrl &= ~PMUC_CHR_AUXADC_SEL_MASK;
    hwp_pmuc->charger_cfg_ctrl |= PMUC_CHR_AUXADC_SEL(3);
    readBat = hal_AnaGpadcGet(HAL_ANA_GPADC_CHAN_6);
    hwp_pmuc->charger_cfg_ctrl &= ~PMUC_CHR_AUXADC_SEL_MASK;
    ...
    {
        return readBat; //((readBat * 59)/1000);
    }
}
#endif /* PMD_CHR_FULL_CHR_BY_CURRENT */
...
PUBLIC VOID pmd_SetChargerStatusHandler(PMD_CHARGER_HANDLER_T handler)
{
    ...
    else
    {
        ...
        hwp_pmu_intf->pmu_intf_icr |=PMU_INTF_AC_ON_INT_CLEAR;
        hwp_pmu_intf->pmu_intf_imr &=~PMU_INTF_AC_ON_INT_MASK;
...
PRIVATE VOID pmd_ForceChargerFinished(BOOL stop)
{
    if (stop)
    {
        ...
        hwp_pmuc->charger_cfg_ctrl &= ~(PMUC_CHR_ENABLE_REG);
        hwp_pmuc->charger_cfg_ctrl |= PMUC_CHR_ENABLE_DR;
    }
    else
    {
        // Direct-reg to enable charging
        // Bypass PMU charge timeout control (about 8 hours)
        hwp_pmuc->charger_cfg_ctrl |= (PMUC_CHR_ENABLE_DR | PMUC_CHR_ENABLE_REG);
    }
}
...
PRIVATE VOID pmd_RestoreChargerControl(VOID)
{
    ...
    hwp_pmuc->charger_cfg_ctrl &= ~(PMUC_CHR_ENABLE_REG | PMUC_CHR_ENABLE_DR);
}
...
PROTECTED RDA_PMU_CHR_STATUS_T pmd_GetChargerHwStatus(VOID)
{
    ...
    if ((hwp_pmuc->ana_status & PMUC_CHR_AC_ON) == 0)
    { 
...
PROTECTED PMD_CHARGER_STATUS_T pmd_ChargerChargingState(RDA_PMU_CHR_STATUS_T status)
{
    ...
    //meanBattVolt large than Li-xx battery max volt, stop charge directly.
    if(meanBattVolt+PMD_MV_OFFSET_CHG_FULL_COMPENSATION > PMD_Li_BATTERY_MAX_VOLTAGE)
    {
...
#ifdef PMD_CHR_FULL_CHR_BY_CURRENT
    //Judge full charge with current
    isFullCharged = (meanBattVolt+PMD_MV_OFFSET_CHG_FULL_COMPENSATION >= g_pmdConfig->batteryLevelFullMV)
                && (pmd_GetGpadcChargerCurrent() < 5);
    PMD_TRACE(PMD_DBG_TRC, 0, "pmd_ChargerChargingState meanBattVolt=%d, g_pmdChargingStage=%d, Curr%d", 
            meanBattVolt,g_pmdChargingStage, pmd_GetGpadcChargerCurrent());

#elif defined(PMD_CHR_FULL_CHR_BY_TERM)
    isFullCharged = (meanBattVolt+PMD_MV_OFFSET_CHG_FULL_COMPENSATION >= g_pmdConfig->batteryLevelFullMV)
                && ((hwp_pmuc->ana_status&PMUC_CHR_TERM) != 0);
    PMD_TRACE(PMD_DBG_TRC, 0, "pmd_ChargerChargingState meanBattVolt=%d, g_pmdChargingStage=%d", 
            meanBattVolt,g_pmdChargingStage);
#else
    isFullCharged = (meanBattVolt+PMD_MV_OFFSET_CHG_FULL_COMPENSATION >= g_pmdConfig->batteryLevelFullMV);

    PMD_TRACE(PMD_DBG_TRC, 0, "pmd_ChargerChargingState meanBattVolt=%d, g_pmdChargingStage=%d", meanBattVolt,g_pmdChargingStage);
#endif /* PMD_CHR_FULL_CHR_BY_CURRENT */
...
#ifdef PMD_CHR_HIGH_CAPACITY_BATTERY
...
#endif /* PMD_CHR_HIGH_CAPACITY_BATTERY */
            {
                // Fast charging
...
#ifdef KEEP_FULL_STATE_IN_RECHARGE    //5856e has independ charging led, keep full state in recharge will light two led
            if (nextState != PMD_CHARGER_FULL_CHARGE)
#endif /* KEEP_FULL_STATE_IN_RECHARGE */
            {
    ...
    case PMD_CHARGING_STAGE_ON:
        if(isFullCharged)
        {
            ...
            //Fix invalid g_pmdChargerOnInterval
            if(0 == g_pmdChargerOnInterval)
                g_pmdChargerOnInterval = PMD_INTERVAL_CHG_ON_LONG;

            if (g_pmdChargerFirstVoltageCheck ||
                curTime - g_pmdChargerMeasThreshReachedTime >= g_pmdChargerOnInterval)
            {
                // Stop charge when battery full imidiately in 5856E
                g_pmdChargingStage = PMD_CHARGING_STAGE_OFF;
                // Set next state to full
                nextState = PMD_CHARGER_FULL_CHARGE;
                // Force to disable charging
...
PROTECTED VOID pmd_ChargerManager(PMD_CHARGE_EVENT_T event)
{
    ...
    PMD_TRACE(PMD_INFO_TRC, 0, "charger: nextState=%d ,oldstatus=%d, pulseCnt=%d curBattVolt=%d, AC=%d,Curr=%d",
                        nextState, g_pmdChargerState.status, g_pmdChargerState.pulsedOnCount, pmd_GetGpadcBatteryLevel(), pmd_GetGpadcChargerLevel(), pmd_GetGpadcChargerCurrent());
...
{
    UINT16 ret_value = 0;
    UINT32 scStatus, effuse_ctrl1;

    scStatus = hal_SysEnterCriticalSection();
    
    effuse_ctrl1 = hwp_pmu_intf->efuse_ctrl1;//record effuse_ctrl1
    hwp_pmu_intf->efuse_ctrl1 = 0x70000000; //we should clean effuse first
    //select page
    hwp_pmu_intf->efuse_ctrl1 |= PMU_INTF_EFUSE_SEL_WORD_REG(page)| PMU_INTF_EFUSE_SEL_WORD_DR;
    hwp_pmu_intf->efuse_ctrl1 |= PMU_INTF_EFUSE_SENSE_REG|PMU_INTF_EFUSE_SENSE_EN_REG;
    //we should first set PMU_INTF_EFUSE_SENSE_EN_DR!!!!
    hwp_pmu_intf->efuse_ctrl1 |= PMU_INTF_EFUSE_SENSE_EN_DR;
    hwp_pmu_intf->efuse_ctrl1 |= PMU_INTF_EFUSE_SENSE_DR;

    ret_value = (hwp_pmu_intf->efuse_ctrl1)&PMU_INTF_EFUSE_OUT_MASK;

    switch(page)
    {
        case PMD_EFFUSE_PAGE_AUDIO_DIFF_L:
    		//TODO:
    		break;
    	...
    	case PMD_EFFUSE_PAGE_VBAT_4P2:
    		ret_value = (ret_value<<2) |0x3; //Bit9~2, and set bit 0,1 to reduce errors
    		break;

    	case PMD_EFFUSE_PAGE_VBAT_2P7:
    		ret_value = (ret_value<<2) |0x3;
    		break;

    	case PMD_EFFUSE_PAGE_AUDIO_DIFF_R:
            //TODO:
            break;
        case PMD_EFFUSE_PAGE_KEYSENSE_0K:
			break;
        case PMD_EFFUSE_PAGE_KEYSENSE_OHM_10K:
            //TODO:
		    break;
		case PMD_EFFUSE_PAGE_KEYSENSE_OHM_10K2:
			break;
        case PMD_EFFUSE_PAGE_KEYSENSE_68K:
            //TODO:
		    break;    
    ...
    //restore effuse_ctrl1
    hwp_pmu_intf->efuse_ctrl1 = effuse_ctrl1;
    hal_SysExitCriticalSection(scStatus);
...
UINT32 pmd_keyCalibValue(UINT8 index)
{
	...
	return pmd_GetEffuseValue(calibEnum[index]);
}
UINT32 pmd_GetEncryptUid()
{
	UINT32 val =(pmd_GetEffuseValue(PMD_EFFUSE_PAGE_VBAT_4P2)&0x3FF)<<20;	
	val +=(pmd_GetEffuseValue(PMD_EFFUSE_PAGE_VBAT_2P7)&0x3FF)<<10;
...
PRIVATE VOID pmd_GetEffuseGpadc(VOID)
{
    ...
    *hal_AnaGpadcGetCalibLow() = pmd_GetEffuseValue(PMD_EFFUSE_PAGE_VBAT_2P7);
}
...
PUBLIC VOID pmd_InitCharger(VOID)
{
    ...
    //hwp_pmuc->pwr_sys_ctrl |= PMUC_CHR_AC_ON_RESET_BYPASS;
    ...
    UINT32 chr_pre_ibit = 3;//PMU_GET_BITFIELD(effuse, RDA_PMU_CHR_PRE_IBIT_REG);
    UINT32 chr_vfb_sel = PMU_GET_BITFIELD(effuse, RDA_PMU_EFUSE_CHR_VFB_SEL_REG);
#if 1
	...
	//Must set SEL_DR before set vfb
    hwp_pmuc->charger_cfg_ctrl |= PMUC_CHR_VFB_SEL_DR;

    reg = hwp_pmuc->charger_cfg_ctrl;
    
    //pmd_OpalSpiWrite(RDA_ADDR_CHARGER_SETTING2, g_pmdChargerSetting2);
    reg &= ~PMUC_CHR_VFB_SEL_REG_MASK;
    reg |= PMUC_CHR_VFB_SEL_REG(chr_vfb_sel);
    //set pre charge current 46mA
    reg &= ~PMUC_CHR_PRE_I_BIT_MASK;
    reg |= PMUC_CHR_PRE_I_BIT(chr_pre_ibit);

    hwp_pmuc->charger_cfg_ctrl = reg;
...
#elif defined(PMD_CHR_FULL_CHR_BY_TERM)
    //Set pre_charge mode
    if(1)//PMUC_CHR_CC_MODE_PRE_CHARGE_MODE != hwp_pmuc->charger_cfg_ctrl & PMUC_CHR_CC_MODE)
    {
        hwp_pmuc->charger_cfg_ctrl |= PMUC_CHR_CC_MODE_DR;
        hwp_pmuc->charger_cfg_ctrl &= ~PMUC_CHR_CC_MODE_REG;
    }
#else
#endif /* PMD_CHR_HIGH_CAPACITY_BATTERY */
    ...
    hal_HstSendEvent(SYS_EVENT, 0x20180808);
    hal_HstSendEvent(SYS_EVENT, status);
    hal_HstSendEvent(SYS_EVENT, effuse);

    if (status == CHR_STATUS_AC_ON)
    {
