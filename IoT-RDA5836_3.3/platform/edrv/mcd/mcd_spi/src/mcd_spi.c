...
PRIVATE MCD_ERR_T mcd_SendCommand(UINT8 cmdId, UINT32 argument,UINT16* status,UINT16 dataSize,UINT8** pdata_array)
{
    ...
    //printf("add = %d,val=%d",g_SpiMcdRxBuf,g_SpiMcdRxBuf[0] );
    //printf(" 1 g_SpiMcdRxBuf[0]=%d", g_SpiMcdRxBuf[0]);
    hal_HstSendEvent(0xff, g_SpiMcdRxBuf[0]);
    if (g_dmaBusy==0)
    {
        return MCD_ERR_DMA_BUSY;
    }
        //printf("2 g_SpiMcdRxBuf[0]=%d", i,g_SpiMcdRxBuf[0]);
    //hal_HstSendEvent(0xff, g_SpiMcdRxBuf[0]);
    g_dmaBusy=hal_SpiSendData(HAL_SPI, g_mcdSpiCs, (UINT8*) g_SpiMcdRdCmdBuf, g_transferSize);
        //printf(" 3 g_SpiMcdRxBuf[0]=%d", g_SpiMcdRxBuf[0]);
    //sxr_Sleep(SLEEP_DURATION_STD);
    mcd_TimeDelaly(SLEEP_DURATION_STD);
    //hal_HstSendEvent(0xff, g_SpiMcdRxBuf[0]);
    //printf("4 g_SpiMcdRxBuf[0]=%d", g_SpiMcdRxBuf[0]);
    if (g_dmaBusy == 0)
    {
        ...
        //printf("5 g_SpiMcdRxBuf[0]=%d", g_SpiMcdRxBuf[0]);
     //hal_HstSendEvent(0xff, g_SpiMcdRxBuf[0]);
    printf("#12");
    //  printf("6 g_SpiMcdRxBuf[0]=%d", g_SpiMcdRxBuf[0]);
    ...
    //  printf("7 g_SpiMcdRxBuf[0]=%d", g_SpiMcdRxBuf[0]);
    ...
    if ( cmdId == 0xff )
    {
        ...
        printf("i=%d,g_SpiMcdRxBuf[i]=%d", i,g_SpiMcdRxBuf[i]);
    }
    else
    {
...
PUBLIC MCD_ERR_T mcd_GetCardSize(MCD_CARD_SIZE_T* size)
{
    ...
    //TO BE DONE
    size->blockLen =  0; //g_mcdBlockLen;
    size->nbBlock  = 0; //g_mcdNbBlock;
        ...
        if (errorStatus == MCD_ERR_NO)
        {
            ...
            if ((cmdStatus & 0xff) == 0x1)
            {
                ...
                // Without CMD8, we can't continue
                {
                    ...
                    MCD_TRACE(MCD_INFO_TRC, 0,"CMD8 cmdStatus = %d", cmdStatus);
                    if ((cmdStatus & 0xFB) == 0x1)
                    {
                        ...
                        {
                             MCD_TRACE(MCD_INFO_TRC, 0, "This card   comply to the spec version 2.0");
                        }
                        // If in Idle state, continue procedure
                        MCD_TRACE(MCD_INFO_TRC, 0, "MMC/SD in IDLE State. CMD0 sent %d times", j+1);
                        End_Init = TRUE;
...
//hal_HstSendEvent(0x22220000);
    ...
    MCD_TRACE(MCD_INFO_TRC, 0,"mcd_Open finished.");

#if 0
    static unsigned char testData[512] = {0};
    static unsigned char readData[512] = {0};

    for (int i = 0; i < 512; i++)
        testData[i] = i; 

    errorStatus = mcd_Write(512*50,testData, 0);
    if (errorStatus != MCD_ERR_NO) {
        printf("Write err");
        return MCD_ERR; 
    }
    //mcd_TimeDelaly(10000);
    mcd_Read(512*50,readData,0);
    for (int i = 0; i < 512; i++) {
        if (testData[i] != readData[i]) {
            printf("test i = %d is different: t[%d]-r[%d]", i,testData[i],readData[i]);
            break;
        }
...
MCD_ERR_T mcd_Read(UINT32 startAddr, UINT8* rdDataBlock, UINT32 blockLength)
{
    ...
    mcd_TimeDelaly(SLEEP_DURATION_READ);
    ...
    mcd_TimeDelaly(SLEEP_DURATION_RD);
