...
#endif
#if defined(LINEIN_RECORDER_SUPPORT)
#define STATIC_TX_BUF_LEN		1024 //1024
#else
#if BLUETOOTH_USE_MARLIN==1
#define STATIC_TX_BUF_LEN		1024
#else
#define STATIC_TX_BUF_LEN		256 	
#endif
#endif /* LINEIN_RECORDER_SUPPORT */
...
DEVICE *Uart_Dev[2]={NULL};

 INT32       fileHandle=0;
#ifdef BT_UART_BREAK_INT_WAKEUP
BOOL g_Uart1BreakDetected = FALSE;
BOOL g_Uart2BreakDetected = FALSE;
#else
HAL_GPIO_GPIO_ID_T g_GpioId= NULL;
HAL_GPIO_IRQ_MASK_T gpioMaskCfg= {NULL};
#endif
VOID(*g_func)(VOID) = NULL;
VOID(*g_func2)(VOID) = NULL;

void Uart1_IrqHandler (HAL_UART_IRQ_STATUS_T status, HAL_UART_ERROR_STATUS_T error) ;
#if BLUETOOTH_USE_MARLIN==1	
void Uart2_marlin_IrqHandler (HAL_UART_IRQ_STATUS_T status, HAL_UART_ERROR_STATUS_T error) ;
#endif
...
VOID UART2_Register_Wakeup_Cb(void (*cb)())
{
    g_func2 = cb;
}
...
#if defined(LINEIN_RECORDER_SUPPORT)
       pDev->writeBuf.Buf_len=4096;	// tx buff not need big
#else
#if BLUETOOTH_USE_MARLIN==1
       pDev->writeBuf.Buf_len=1024;	// tx buff not need big
#else
		pDev->writeBuf.Buf_len=256;	
#endif	   
#endif /* LINEIN_RECORDER_SUPPORT */
...
#if BLUETOOTH_USE_MARLIN==1
       pDev->readBuf.Buf_len=4096; // rx buff need 1K
#else
		pDev->readBuf.Buf_len=1024;
#endif
...
//	SXS_TRACE(TSTDOUT,"UART_Open: pDev->port = %d, pDev->writeBuf = 0x%x, pDev->readBuf=0x%x\n",pDev->port,pDev->writeBuf,pDev->readBuf);				
...
#if defined(LINEIN_RECORDER_SUPPORT)
			SRAM_BUF.tx_buf=(UINT8*)COS_Malloc_NoCache(4096);
#else
        ...
        {
            SRAM_BUF2.rx_buf1=(UINT8*)COS_Malloc_NoCache(STATIC_RX_BUF_LEN);
            if(NULL==SRAM_BUF2.rx_buf1)
                goto _openError;


            SRAM_BUF2.tx_buf=(UINT8*)COS_Malloc_NoCache(STATIC_TX_BUF_LEN);
            if(NULL==SRAM_BUF2.tx_buf)
                goto _openError;
        }
...
#endif	
	 hal_HstSendEvent(SYS_EVENT, 0x13062222);
 	 return TRUE ;
_openError:
    //UART_Close(port);
	return FALSE;
...
void UART_SetDCBConfig(PORT port, UINT32 band, UINT8 flowctrl )
{
...
#if BLUETOOTH_USE_MARLIN==1	 
	Uart_ISR[1]=Uart2_marlin_IrqHandler;
#endif	
    ...
    // critical section.
    if (g_Uart1BreakDetected && pDev->port == 1)
    {
        hal_UartIrqEnableBreakInt(pDev->port, FALSE);
    }
    if (g_Uart2BreakDetected && pDev->port == 2)
    {
        hal_UartIrqEnableBreakInt(pDev->port, FALSE);
    }
#endif
	 if(pDev->RxUseDMA && port==BT_UART_PORT )
	 {	
        ...
        if(pDev->RxUseDMA && port==BT_UART_PORT2)
	 {	
		 
	  	hal_UartGetData (pDev->port, SRAM_BUF2.rx_buf1, STATIC_RX_BUF_LEN);	
	  	pDev->RxPending = 1;
	 }
...
BOOL UART_Close(PORT port)
{
    ...
    hal_UartBreakIntWakeup(pDev->port,FALSE);
	  ...
	  pDev->port=0;
        ...
        if(port==BT_UART_PORT2)
        {
            if(SRAM_BUF2.rx_buf1!=NULL)
            {
            	COS_Free(SRAM_BUF2.rx_buf1);
            	SRAM_BUF2.rx_buf1=NULL;
            }

            if(SRAM_BUF2.tx_buf!=NULL)
            {
            	COS_Free(SRAM_BUF2.tx_buf);
            	SRAM_BUF2.tx_buf=NULL;
            }
        }
...
void DealWith_UartRxIrq(DEVICE *pDev,BOOL  DMA_used, BOOL  Timeout)
{
   UINT32 uDataSize, real_len=0;
    UINT8 *rxDmaBufEmpty = NULL, *data;
   // UINT8 *rxDmaBufLoad;
       if(pDev->port==BT_UART_PORT)
            rxDmaBufEmpty =  SRAM_BUF.rx_buf1 ;

        if(pDev->port==BT_UART_PORT2)
            rxDmaBufEmpty =  SRAM_BUF2.rx_buf1 ;
...
//#if (CHIP_ASIC_ID == CHIP_ASIC_ID_GALLITE) && defined(GALLITE_IS_8805)
            ...
            if(pDev->port == BT_UART_PORT2)
            {
                real_len=get_data_from_CircularBuffer(&pDev->writeBuf, SRAM_BUF2.tx_buf,UART_FIFO_LEN);

                if(real_len>0)
                {	
                    hal_UartSendData(pDev->port, SRAM_BUF2.tx_buf, real_len);	

                    SXS_TRACE(TSTDOUT,"tx2 laixf %x,%x,Normal ",real_len,DMA_used );
                    //SXS_TRACE(TSTDOUT," %x,%x,%x", SRAM_BUF.tx_buf[0],SRAM_BUF.tx_buf[1],SRAM_BUF.tx_buf[2]);
                }
            }
...
#if BLUETOOTH_USE_MARLIN==1	
void Uart2_marlin_IrqHandler (HAL_UART_IRQ_STATUS_T status, HAL_UART_ERROR_STATUS_T reg) 
{
///SXS_TRACE(TSTDOUT,"Uart1_IrqHandler  status = %d   ",status);
...
//qiff add for check uart DMA status
	...
	if(pDev->TxUseDMA)
	{
            ...
            {   
                while ( (!(hal_UartTxFinished(pDev->port)) )||(!(hal_UartTxDmaDone( pDev->port ) ))  );
                send_rel_len=get_data_from_CircularBuffer(&pDev->writeBuf, SRAM_BUF2.tx_buf,STATIC_TX_BUF_LEN);

                 //pDebugPrintfEX((pLOGDEBUG,pLOGHCI,"DMA UART_PutBytes send_rel_len=%d", send_rel_len));

                if(send_rel_len >0 )
                {
                    NumToWrite=hal_UartSendData(pDev->port, SRAM_BUF2.tx_buf, send_rel_len);
                }
            }
	...
	else
	{
			...
			while(!Is_CircularBuffer_Empty(&pDev->writeBuf))
			{
				...
				Tx_fifo_avail=hal_UartTxFifoAvailable(pDev->port);
                            ...
                            if(port==BT_UART_PORT2)
                            {
                                send_rel_len=get_data_from_CircularBuffer(&pDev->writeBuf, SRAM_BUF2.tx_buf,Tx_fifo_avail);
                                //pDebugPrintfEX((pLOGDEBUG,pLOGHCI,"UART_PutBytes send_rel_len=%d", send_rel_len));

                                hal_UartSendData(pDev->port, SRAM_BUF2.tx_buf, send_rel_len);
                            }
