...
export BTCPUGEN := no
ifeq "${CHIP_HAS_BTCPU_ROM}" "1"
ifneq "${NO_BTCPU_IMAGE}" "1"
    BTCPUGEN := yes
endif
endif
...
export AR := armar
export OBJCOPY := echo 
export OBJDUMP := fromelf
export NM :=
export LD := armlink

# for compitable with gcc
...
ifeq "${CHIP_CPU_TYPE}" "arm"
export LIBGCC_PATH := ${SOFT_WORKDIR}/env/lib/gcc/arm-elf/
endif
...
#TODO
ifeq "${CHIP_CPU_TYPE}" "riscv"
export LIBGCC_PATH := ${SOFT_WORKDIR}/env/lib/gcc/riscv32-unknown-elf/5.4.1
endif

endif   # endif "${COMPILER}" "gcc"
...
ifeq "${CT_RAMRUN}" "1"
export BUILD_DIR := build/download_${LOCAL_BATCH_NAME}_${CT_TARGET}
else
ifeq "${ROM_PROG}" "1"
export BUILD_DIR := build/rom_${CT_TARGET}
else
ifeq "${TFLASH_UPGRADE}" "1"
export BUILD_DIR := build/tflash_upgrade_${CT_TARGET}
else
...
ifeq "${BTCPUGEN}"  "yes"
export BTCPU_BUILD_ROOT := ${BUILD_ROOT}/btcpu
endif
...
# TODO_OPTIM?
export BUILD_TIME := $(shell date "+0x%H%M%S")
ifeq "${BUILD_ROM}" "1"
export BUILD_DATE := $(shell date "+%y%m%d")
else
export BUILD_DATE := $(shell date "+0x%y%m%d")
endif
#export SVN_REVISION := `svn info | grep "Last Changed Rev" | sed 's/.*: //'`
...
ifeq "${BUILD_BTCPU_IMAGE}" "1"
export BUILD_ROOT := ${BTCPU_BUILD_ROOT}
COMPRESS_PROCESS_ENABLE := no
NATIVE_JAVA_SUPPORT := no
endif

# When building RAMINIT image, override BUILD_ROOT, COMPRESS_PROCESS_ENABLE,
# and NATIVE_JAVA_SUPPORT settings
...
INCLUDE_PATH	:= ${MYINCLUDEDIR} -I${LOCAL_INC_DIR} -I${LOCAL_SRC_DIR} ${DEPENDENCY_INCLUDE_PATH}  -I${ROOT_INCLUDE} -I${SOFT_WORKDIR}/target/${CT_TARGET}/${INC_DIR} -I${SOFT_WORKDIR}/platform/chip/comn/${INC_DIR}
...
ifeq "${BTCPUGEN}"  "yes"
ifeq "${BUILD_BTCPU_IMAGE}" "1"
CHIP_ROM_ELF_FILE := ${BTCPU_ROM_ELF_FILE}
CHIP_ROM_DEF_FILE := ${BTCPU_ROM_DEF_FILE}
STRIP_ROM_OBJ2KEEP_FILE := ${STRIP_BTCPU_ROM_OBJ2KEEP_FILE}
STRIP_ROM_OBJ2STRIP_FILE := ${STRIP_BTCPU_ROM_OBJ2STRIP_FILE}
endif
endif
...
USED_ROM_ELF_FILE := ${CHIP_ROM_ELF_FILE}
endif # STRIP_ROM_OBJ2KEEP_FILE or STRIP_ROM_OBJ2STRIP_FILE
ifeq "${LINK_ROM_SYMBOL_ONLY}" "1"
LDFLAG_USED_ELF_FILES := -just-symbols ${SOFT_WORKDIR}/${USED_ROM_ELF_FILE}
...
#----------------------------------------------
# Link BTCPU rom when building XCPU elf
#----------------------------------------------
ifneq "${BTCPU_ROM_ELF_FILE}" ""
...
ifneq "${STRIP_ROM_OBJ2KEEP_FILE}${STRIP_ROM_OBJ2STRIP_FILE}" ""
# BTCPU rom has been stripped when building BCPU elf
USED_BTCPU_ROM_ELF_FILE := ${BTCPU_BUILD_ROOT}/${STRIP_ROM_ELF_FILE}
else
USED_BTCPU_ROM_ELF_FILE := ${SOFT_WORKDIR}/${BTCPU_ROM_ELF_FILE}
endif

ifeq "${BUILD_BTCPU_IMAGE}" "1"
LDFLAG_USED_ELF_FILES += -just-symbols ${SOFT_WORKDIR}/${USED_BTCPU_ROM_ELF_FILE}
endif
...
ifeq "${BUILD_BTCPU_IMAGE}" "1"
LD_SRC := ${SOFT_WORKDIR}/env/link/${BTCPU_LINKER_MASTER}
else
...
ifeq "${CT_RAMRUN}" "1"
SCATTER_FILE := ${SOFT_WORKDIR}/env/link/${CT_ASIC}_flash_prog.txt
else
ifeq "${ROM_PROG}" "1"
SCATTER_FILE := ${SOFT_WORKDIR}/env/link/${CT_ASIC}_rom.txt
else
SCATTER_FILE := ${SOFT_WORKDIR}/env/link/${CT_ASIC}.txt
endif
endif
...
SREC		:= --input-target=elf32-littlearm --output-target=srec
endif

ifeq "${CHIP_CPU_TYPE}" "riscv"
OFORMAT := --oformat=elf32-littleriscv
SREC		:= --input-target=elf32-littleriscv --output-target=srec  --srec-forceS3
endif
...
ifneq "${CT_ASIC}" "5996"
ifneq "${UNIT_TEST_BT}" "1"
LDPPFLAGS += 	-DUNIT_TEST_BT=0 
CFLAGS += 	-DUNIT_TEST_BT=0 
else
LDPPFLAGS += 	-DUNIT_TEST_BT=1 
CFLAGS += 	-DUNIT_TEST_BT=1 
endif
endif
...
LDPPFLAGS += \
	...
	-DIRQ_RAM_SIZE=${IRQ_RAM_SIZE} \
	-DSYS_SRAM_END=${SYS_SRAM_END} \
	-DBTCPU_SYS_SRAM_END=${BTCPU_SYS_SRAM_END} \
	...
	-DBTCPU_IRQ_STACK_SIZE=${BTCPU_IRQ_STACK_SIZE} \
	...
	-DBT_SRAM_BASE=${BT_SRAM_BASE} \
	-DBT_SRAM_SIZE=${BT_SRAM_SIZE} \
	-DBT_ROM_BASE=${BT_ROM_BASE} \
	-DBT_ROM_SIZE=${BT_ROM_SIZE} \
	-DBT_DUALPORT_SRAM_BASE=${BT_DUALPORT_SRAM_BASE} \
	-DBT_DUALPORT_SRAM_SIZE=${BT_DUALPORT_SRAM_SIZE} \
...
LDPPFLAGS += -DCHIP_BTOVERLAY_SUPPORTED=${CHIP_BTOVERLAY_SUPPORTED}
...
ifeq "${RAMINITGEN}"  "yes"
LDPPFLAGS += \
    -DSYS_SRAM_RAMINIT_BASE=${SYS_SRAM_RAMINIT_BASE} \
    -DSYS_SRAM_DATA_SIZE=${SYS_SRAM_DATA_SIZE}
endif
...
LDFLAGS += ${FULL_LIBRARY_FILES}
#LDFLAGS		+= -libpath ${FULL_LIBRARY_PATH} ${ARMLIB} ${MYLIBFILES} ${FULL_LIBRARY_EXT}
...
ifeq "${CHIP_SUBCPU_TYPE}" "arm7tdmi"
ASFLAGS += -mcpu=arm7tdmi
endif

ifeq "${CHIP_SUBCPU_TYPE}" "Cortex-M4"
ASFLAGS += --cpu=Cortex-M4
endif

else #armcc

ifneq "${CHIP_SUBCPU_TYPE}" ""
ASFLAGS += --cpu=${CHIP_SUBCPU_TYPE}
endif
...
ifeq "${CHIP_CPU_TYPE}" "riscv"
# Assembly compilation flags
ASFLAGS += -march=IMXplupv2  -msoft-float
endif
...
ifeq "${UNIT_TEST_FLASH}" "1"
MYCPPFLAGS += -DUNIT_TEST_FLASH=1
else
MYCPPFLAGS += -DUNIT_TEST_FLASH=0
endif
MYCPPFLAGS += -DXCPU_ALL_REG=${XCPU_ALL_REG} 
MYCPPFLAGS += -DGPIO_IDX=${GPIO_IDX} 

ifeq "${NO_SRAM_INIT}" "1"
MYCPPFLAGS += -DNO_SRAM_INIT=1 
else
MYCPPFLAGS += -DNO_SRAM_INIT=0
endif

MYCPPFLAGS += -DCHIP_BTOVERLAY_SUPPORTED=${CHIP_BTOVERLAY_SUPPORTED}
...
ASFLAGS += -g --pd "__MICROLIB SETA 1"
endif # "gcc"
...
# for RISCV, need further investigate 
...
ifeq "${CHIP_CPU_TYPE}" "mips"
CFLAGS += -G0 -Wall -march=xcpu -mtune=xcpu -Wa,-march=xcpu,-mtune=xcpu -EL -mexplicit-relocs -fweb -frename-registers -mmemcpy -mmips-tfile -nostartfiles -nostdlib -nostdinc -nodefaultlibs -fno-builtin -c -pipe 
endif
ifeq "${CHIP_CPU_TYPE}" "arm"
CFLAGS += -Wall  -Wa,  -fweb -frename-registers  -nostartfiles -nostdlib -nostdinc -nodefaultlibs -c -pipe 
endif

ifeq "${CHIP_CPU_TYPE}" "riscv"
CFLAGS += -Wall  -Wa,  -fweb -frename-registers  -nostartfiles -nostdlib -nostdinc -nodefaultlibs -c -pipe
CFLAGS += -mcmodel=medany -static -std=gnu99 -O2 -ffast-math -fno-common -fno-builtin -m32 -march=IMXpulpv2 -mrvc -g -ffunction-sections -fdata-sections
endif
...
ifneq "${CHIP_SUBCPU_TYPE}" ""
CFLAGS += -mcpu=${CHIP_SUBCPU_TYPE}
endif

CFLAGS += ${TARGET_COMPILER_CFLAGS}

else #armcc

CFLAGS = -c -g --cpu=${CHIP_SUBCPU_TYPE}
CFLAGS += ${TARGET_COMPILER_CFLAGS} -DBUILD_DATE=${BUILD_DATE} -DBUILD_TIME=${BUILD_TIME}

SPECIFIC_OBJS := ${SOFT_WORKDIR}/${BUILD_DIR}/platform/chip/${CT_ASIC}/boot/obj/${CT_RELEASE}/*.o
ifeq "${CT_RAMRUN}" "1"
SPECIFIC_OBJS += ${SOFT_WORKDIR}/${BUILD_DIR}/platform/programmer/flash_prog/obj/${CT_RELEASE}/*.o
endif
...
ifeq "${BTCPUGEN}"  "yes"
BTCPU_LODBASE_NO_PATH := btcpu_${LODBASE_NO_PATH}
endif

TFLASH_LODBASE_NO_PATH := tflash_${LODBASE_NO_PATH}
...
ifeq "${BUILD_BTCPU_IMAGE}" "1"
LODBASE_NO_PATH := ${BTCPU_LODBASE_NO_PATH}
endif

# If building RAMINIT image, change the lod base name, but NOT to change BINARY_PATH !
ifeq "${TFLASH_UPGRADE}" "yes"
LODBASE_NO_PATH := ${TFLASH_LODBASE_NO_PATH}
endif
...
AXF_FINAL := ${BAS_FINAL}.axf
RCF_FINAL := ${BAS_FINAL}.rcf
HEXBIN_FINAL := ${BAS_FINAL}.bin
DATA_FINAL  := ${BAS_FINAL}_data.txt
ASM_FINAL := ${BAS_FINAL}.asm
...
ifeq "${BTCPUGEN}"  "yes"
BTCPU_BIN_PATH := ${BTCPU_BUILD_ROOT}/${LOCAL_NAME}
BTCPU_BAS := ${BTCPU_BIN_PATH}/${BTCPU_LODBASE_NO_PATH}
BTCPU_BIN := ${BTCPU_BAS}.elf
BTCPU_BAS_FINAL := ${BINARY_PATH}/${BTCPU_LODBASE_NO_PATH}
BTCPU_BIN_FINAL := ${BTCPU_BAS_FINAL}.elf
BTCPU_MAP_FINAL := ${BTCPU_BAS_FINAL}.map
endif

ifeq "${BUILD_BTCPU_IMAGE}" "1"
BIN_FINAL := ${BTCPU_BIN_FINAL}
MAP_FINAL := ${BTCPU_MAP_FINAL}
endif

BTCPU_HEXBIN_FINAL := ${BINARY_PATH}/${BTCPU_LODBASE_NO_PATH}.bin
BTCPU_BIN_FINAL := ${BINARY_PATH}/${BTCPU_LODBASE_NO_PATH}.elf
TFLASH_HEXBIN_FINAL := ${BINARY_PATH}/${TFLASH_LODBASE_NO_PATH}.bin
TFLASH_BIN_FINAL := ${BINARY_PATH}/${TFLASH_LODBASE_NO_PATH}.elf
...
BT_FLASH_END = `grep _bt_flash_end ${BTCPU_MAP_FINAL} | cut -b 17-35`

LDPPFLAGS += 	-DBT_FLASH_END=${BT_FLASH_END}
endif
endif

ifeq "${LINK_ROM_SYMBOL_ONLY}" "1"
ifneq "${CT_RAMRUN}" "1"
ifneq "${TFLASH_UPGRADE}" "1"
ifneq "${BUILD_BTCPU_IMAGE}" "1"
LDFLAG_USED_ELF_FILES += -just-symbols ${SOFT_WORKDIR}/${TFLASH_BIN_FINAL}
LDPPFLAGS += 	-DTFLASH_IMAGE_SUPPORT=`if [ -f ${TFLASH_BIN_FINAL} ]; then echo 1; else echo 0; fi`
TFLASH_BASE := ${FLASH_SIZE} ${TFLASH_FLASH_SIZE} ${FLASH_BASE}
endif
endif
endif
endif
...
USED_BCPU_ELF_FILE := ${subst ${SOFT_WORKDIR}/,,${BCPU_ELF_FILE_STRIPPED}}
else # !STRIP_BCPU_OBJ2STRIP_FILE
USED_BCPU_ELF_FILE :=  ${subst ${SOFT_WORKDIR}/,,${BCPU_ELF_FILE_FULL}}
...
###############################################################################
# BTCPU elf file (maybe including BTCPU ROM elf file)
###############################################################################
#ifeq "${BTCPUGEN}"  "yes"
ifneq "${BUILD_BTCPU_IMAGE}" "1"
ifneq "${BUILD_ROM}" "1"
# References between BTCPU ROM and XCPU ROM are forbidden at present.
# Note that if XCPU ROM references to BTCPU ROM are allowed,
# 1) when building XCPU ROM, BTCPU ROM image is not in BTCPU_ROM_ELF_FILE_FULL yet,
# but in BTCPU_ELF_FILE_FULL;
# 2) when building XCPU flash image with ROM_IN_FLASH_ELF enabled, BTCPU ROM symbols
# should be stripped from XCPU ROM before linking stage.

ifneq "$(wildcard ${BTCPU_BIN})" ""

BTCPU_ELF_FILE_FULL := ${BTCPU_BIN}
# XCPU flash elf does NOT need to link BTCPU ROM, as it will link BTCPU flash elf,
# and BTCPU ROM symbols have been linked into BTCPU flash elf.

ifneq "${STRIP_BTCPU_OBJ2STRIP_FILE}" ""
STRIP_BTCPU_OBJ2STRIP_FILE_PATHED := ${SOFT_WORKDIR}/${STRIP_BTCPU_OBJ2STRIP_FILE}
BTCPU_ELF_FILE_STRIPPED := ${BTCPU_BIN_PATH}/stripped_BTCPU_elf_file.elf
BTCPU_ROM_ELF_FILE_STRIPPED := ${BTCPU_BIN_PATH}/stripped_BTCPU_rom_elf_file.elf
XCPU_ELF_FILE_STRIPPED := ${BTCPU_BIN_PATH}/XCPU_elf_file.elf-stripped
USED_BTCPU_ELF_FILE := ${subst ${SOFT_WORKDIR}/,,${BTCPU_ELF_FILE_FULL}}
else # !STRIP_BTCPU_OBJ2STRIP_FILE
USED_BTCPU_ELF_FILE :=  ${subst ${SOFT_WORKDIR}/,,${BTCPU_ELF_FILE_FULL}}
endif # !STRIP_BTCPU_OBJ2STRIP_FILE

# Load the content of BTCPU elf file into XCPU elf
LDPPFLAGS += -DLOAD_BTCPU_ELF
LDFLAG_USED_ELF_FILES += # ${USED_BTCPU_ELF_FILE}  ${BTCPU_ROM_ELF_FILE}

endif # BTCPU_BIN

endif # !BUILD_ROM
endif # !BUILD_BTCPU_IMAGE
#endif # BTCPUGEN
# ------------------------------
# End of BTCPU elf file
# ------------------------------
...
USED_RAMINIT_ELF_FILE := ${subst ${SOFT_WORKDIR}/,,${RAMINIT_ELF_FILE_FULL}}
...
ifneq "${IS_ENTRY_POINT}" "yes"
	...
	rm -f ${HEX} ${MAP} ${BIN}
	# Remove files in hex.
	rm -f ${RCF_FINAL} ${HEX_FINAL} ${MAP_FINAL} ${BIN_FINAL} ${LODBASE}*.lod ${XCP_FINAL} ${XMD_FINAL} ${DIS_FINAL}
endif # IS_ENTRY_POINT
...
ifeq "${BTCPUGEN}"  "yes"
	${MAKE} hexcleanstem BUILD_BTCPU_IMAGE=1
endif
ifeq "${RAMINITGEN}"  "yes"
...
ifeq "${BTCPUGEN}"  "yes"
	${MAKE} allcleanstem BUILD_BTCPU_IMAGE=1
endif
ifeq "${RAMINITGEN}"  "yes"
...
ifeq "${BTCPUGEN}"  "yes"
	${MAKE} cleanstem BUILD_BTCPU_IMAGE=1
endif
ifeq "${RAMINITGEN}"  "yes"
...
ifeq "${BTCPUGEN}"  "yes"
	${MAKE} depcleanstem BUILD_BTCPU_IMAGE=1
endif
ifeq "${RAMINITGEN}"  "yes"
...
ifeq "${BTCPUGEN}"  "yes"
	${MAKE} xcprealcleanstem BUILD_BTCPU_IMAGE=1
endif
ifeq "${RAMINITGEN}"  "yes"
...
ifeq "${BTCPUGEN}"  "yes"
	${MAKE} xcpfinalcleanstem BUILD_BTCPU_IMAGE=1
endif
ifeq "${RAMINITEN}"  "yes"
...
${OBJ_REL_PATH}/${patsubst %.c,%.o,${VERSION_SRC}} : updateversion
...
ifeq "${CHIP_CPU_TYPE}" "arm"
	cd ${BIN_PATH} && ${OBJCOPY} -I binary -O elf32-littlearm -B arm $*.bin.lzma $@
endif

ifeq "${CHIP_CPU_TYPE}" "riscv"
	cd ${BIN_PATH} && ${OBJCOPY} -I binary -O elf32-littleriscv -B riscv $*.bin.lzma $@
endif
...
ifeq "${CHIP_CPU_TYPE}" "arm"
	${OBJCOPY} -I binary -O elf32-littlearm -B arm --rename-section .data=.user_data,alloc,load,readonly,data,contents $< $@
endif

ifeq "${CHIP_CPU_TYPE}" "riscv"
	${OBJCOPY} -I binary -O elf32-littleriscv -B riscv --rename-section .data=.user_data,alloc,load,readonly,data,contents $< $@
endif
...
${OBJ_REL_PATH}/%.o: ${LOCAL_SRC_DIR}/%.S
	@${ECHO} "CPP               $*.S"
	$(CPP) $(CPPFLAGS) $(ASCPPFLAGS)  -MT ${OBJ_REL_PATH}/$*.o -MD -MP -MF ${DEPS_REL_PATH}/$*.d -o ${OBJ_REL_PATH}/$*.asm $(SOFT_WORKDIR)/$(LOCAL_NAME)/$(REALPATH)
	@${ECHO} "AS                $*.asm"
	$(AS) $(ASFLAGS) -o ${OBJ_REL_PATH}/$*.o ${OBJ_REL_PATH}/$*.asm

${OBJ_REL_PATH}/%.o: ${LOCAL_SRC_DIR}/%.c
	@${ECHO} "CC                $*.c"
	$(CC) -MT ${OBJ_REL_PATH}/$*.o -MD -MP -MF ${DEPS_REL_PATH}/$*.d $(C_SPECIFIC_CFLAGS) $(CFLAGS) $(CT_MIPS16_CFLAGS) $(MYCFLAGS) $(CPPFLAGS)  -o ${OBJ_REL_PATH}/$*.o $(SOFT_WORKDIR)/$(LOCAL_NAME)/$(REALPATH)

${OBJ_REL_PATH}/%.o: ${LOCAL_SRC_DIR}/%.cpp
	@${ECHO} "C++               $*.cpp"
	$(C++) -MT ${OBJ_REL_PATH}/$*.o -MD -MP -MF ${DEPS_REL_PATH}/$*.d $(C++_SPECIFIC_CFLAGS) $(CFLAGS) $(CT_MIPS16_CFLAGS) $(MYCFLAGS) $(CPPFLAGS)  -o ${OBJ_REL_PATH}/$*.o $(SOFT_WORKDIR)/$(LOCAL_NAME)/$(REALPATH) $(EXTERN_CPPFLAGS)
else
${OBJ_REL_PATH}/%.o: ${LOCAL_SRC_DIR}/%.S
	${CPP} $(CPPFLAGS) -o ${OBJ_REL_PATH}/$*.asm $(SOFT_WORKDIR)/$(LOCAL_NAME)/$(REALPATH)
	@${ECHO} "AS                $*.asm"
	$(AS) $(ASFLAGS) -o ${OBJ_REL_PATH}/$*.o ${OBJ_REL_PATH}/$*.asm

${OBJ_REL_PATH}/%.o: ${LOCAL_SRC_DIR}/%.c
	@${ECHO} "CC                $*.c"
	$(CC) --mm $(CFLAGS) $(MYCFLAGS) ${MYCPPFLAGS} $(CPPFLAGS) ${INCLUDE_PATH} $(SOFT_WORKDIR)/$(LOCAL_NAME)/$(REALPATH) > ${DEPS_REL_PATH}/$*.d
	depgen.sh ${DEPS_REL_PATH}/$*.d ${OBJ_REL_PATH} $*.o
	$(CC) $(CFLAGS) $(MYCFLAGS) ${MYCPPFLAGS} $(CPPFLAGS) ${INCLUDE_PATH} -o ${OBJ_REL_PATH}/$*.o $(SOFT_WORKDIR)/$(LOCAL_NAME)/$(REALPATH)
...
ifneq "${TFLASH_UPGRADE}" "1"
ifneq "${CT_RAMRUN}" "1"
LD_OPTIONAL_OBJECTS += ${BUILD_ROOT}/res.bin.o
endif
endif
endif
endif
...
ifeq "${COMPILER}" "armcc"
ifeq "${BUILD_ROM}" "1"
TARGET_LINKER_CFLAGS += "--symdefs=${BAS}_sym.o"
ifeq "${COMBINED_HOST}" "1"
CORE_LIBS = $(shell ls $(SOFT_WORKDIR)/platform/service/bt/btadp/libbtllrom*.a -r)
CORE_LIB = $(word 1, ${CORE_LIBS})

LD_OPTIONAL_OBJECTS += ${CORE_LIB}
ifeq "${BT_SUPPORT_RWIP}" "1"
TARGET_LINKER_CFLAGS += --keep co_* --keep ecc_*
endif # "${BT_SUPPORT_RWIP}" "1"
TARGET_LINKER_CFLAGS += --keep boot_*
endif # "${COMBINED_HOST}" "1"
else # "${BUILD_ROM}" "0"
LD_OPTIONAL_OBJECTS += $(SOFT_WORKDIR)/${CHIP_ROM_SYM_FILE}
ifeq "${CT_RAMRUN}" "1"
TARGET_LINKER_CFLAGS += --remove --entry boot_Sector
else
TARGET_LINKER_CFLAGS += --remove --entry 0x1200011
endif
endif
endif
...
ifneq "${STRIP_ROM_ELF_FILE_PATHED}" ""
...
#	echo ${LINK_ROM_ELF} "${LOCAL_NAME}" "${BTCPU_BIN}" "${BTCPU_LODBASE_NO_PATH}" "${LODBASE_NO_PATH}"


# Keep RAMINIT (and possibly ROM) contents in flash elf file
ifneq "${USED_RAMINIT_ELF_FILE}" ""
	${call GEN_ROM_ELF_LD_FILE_FUNC,${USED_RAMINIT_ELF_FILE}} > ${subst ${SOFT_WORKDIR}/,,${BUILD_ROOT}}/link_raminit_elf.ld
else # USED_RAMINIT_ELF_FILE == ""
# Keep ROM contents in flash elf file
ifeq "${LINK_ROM_ELF}" "1"
ifneq "${LINK_ROM_SYMBOL_ONLY}" "1"
ifneq "${USED_ROM_ELF_FILE}" ""
	${call GEN_ROM_ELF_LD_FILE_FUNC,${subst ${SOFT_WORKDIR}/,,${USED_ROM_ELF_FILE}}} > ${subst ${SOFT_WORKDIR}/,,${BUILD_ROOT}}/link_rom_elf.ld
...
ifneq "${USED_BCPU_ELF_FILE}" ""
	${call GEN_COMMON_ELF_LD_FILE_FUNC,${USED_BCPU_ELF_FILE}} > ${subst ${SOFT_WORKDIR}/,,${BUILD_ROOT}}/link_bcpu_elf.ld
endif
endif

# Load BTCPU elf contents into XCPU flash elf file
ifneq "${BUILD_ROM}" "1"
ifneq "${USED_BTCPU_ELF_FILE}" ""
#	${call GEN_COMMON_ELF_LD_FILE_FUNC,${BTCPU_ROM_ELF_FILE}} > ${subst ${SOFT_WORKDIR}/,,${BUILD_ROOT}}/link_btcpu_rom_elf.ld
endif
endif
...
ifeq "${COMPILER}" "armcc"
	${CC} -c $(CFLAGS) $(MYCFLAGS) ${MYCPPFLAGS} $(CPPFLAGS) ${INCLUDE_PATH} ${SOFT_WORKDIR}/target/${CT_TARGET}/resource/res.bin.c -o ${BUILD_ROOT}/res.bin.o
	${CPP} ${LDPPFLAGS} ${LD_SRC} > ${LD_FILE}
	${LD} $(LDFLAGS) ${SPECIFIC_OBJS} ${LD_OPTIONAL_OBJECTS} ${TARGET_LINKER_CFLAGS} --datacompressor=off --scatter ${LD_FILE} --list ${MAP_FINAL}\
        	--map -o ${AXF}
		
	${CP} ${AXF} ${AXF_FINAL}
	fromelf  ${AXF_FINAL} --bin --output ${HEXBIN_FINAL}
	fromelf  ${AXF_FINAL} --vhx --32x1 -o ${RCF_FINAL}

	fromelf -rc ${AXF_FINAL} > ${ASM_FINAL}
	fromelf -ad ${AXF_FINAL} > ${DATA_FINAL}
else  # "armcc"

# Generate resource o
ifneq "${WIFI_MODEL}" "mcuwifi"
ifneq "${CHIP_CPU_TYPE}" "riscv" 
	${OBJCOPY} -I binary -O elf32-littlemips -B mips ${SOFT_WORKDIR}/target/${CT_TARGET}/resource/res.bin ${BUILD_ROOT}/res.bin.o
else
	${CC} -c $(CFLAGS) $(MYCFLAGS) ${MYCPPFLAGS} $(CPPFLAGS) ${INCLUDE_PATH} ${SOFT_WORKDIR}/target/${CT_TARGET}/resource/res.bin.c -o ${BUILD_ROOT}/res.bin.o
endif
endif
...
ifeq "${CHIP_HAS_BTCPU}" "1"
	...
	${CP} ${BIN} ${BINARY_PATH}/xcpu-${subst ${BINARY_PATH}/,,${BIN_FINAL}}
	${CP} ${MAP} ${BINARY_PATH}/xcpu-${subst ${BINARY_PATH}/,,${MAP_FINAL}}

	${OBJCOPY} --strip-symbols=${STRIP_BTCPU_OBJ2STRIP_FILE_PATHED} \
						 --remove-section .bt_dualport_sram_bss \
							   ${USED_BTCPU_ELF_FILE} \
							   ${BTCPU_ELF_FILE_STRIPPED} >/dev/null 2>&1 
	${OBJCOPY} --strip-symbols=${STRIP_ROM_OBJ2STRIP_FILE_PATHED} \
							   ${BIN} \
							   ${XCPU_ELF_FILE_STRIPPED}
	${OBJCOPY} --strip-symbols=${STRIP_ROM_OBJ2STRIP_FILE_PATHED} \
							   ${BTCPU_ROM_ELF_FILE} \
							   ${BTCPU_ROM_ELF_FILE_STRIPPED}
	${OBJCOPY} --remove-section .rom_usb_bss \
							   ${CHIP_ROM_ELF_FILE} \
							   ${STRIP_ROM_ELF_FILE_PATHED}

	${call GEN_COMMON_ELF_LD_FILE_FUNC,${subst ${SOFT_WORKDIR}/,,${BTCPU_ELF_FILE_STRIPPED}}} > ${subst ${SOFT_WORKDIR}/,,${BUILD_ROOT}}/link_btcpu_elf.ld
	${call GEN_COMMON_ELF_LD_FILE_FUNC,${subst ${SOFT_WORKDIR}/,,${XCPU_ELF_FILE_STRIPPED}}} > ${subst ${SOFT_WORKDIR}/,,${BUILD_ROOT}}/link_xcpu_elf.ld
	${call GEN_COMMON_ELF_LD_FILE_FUNC,${subst ${SOFT_WORKDIR}/,,${BTCPU_ROM_ELF_FILE_STRIPPED}}} > ${subst ${SOFT_WORKDIR}/,,${BUILD_ROOT}}/link_btcpu_rom_elf.ld
	${call GEN_ROM_ELF_LD_FILE_FUNC,${subst ${SOFT_WORKDIR}/,,${STRIP_ROM_ELF_FILE_PATHED}}} > ${subst ${SOFT_WORKDIR}/,,${BUILD_ROOT}}/link_rom_elf.ld
  
	${LDPP} ${LDPPFLAGS} ${SOFT_WORKDIR}/env/link/iota_master_flash.ld > ${LD_FILE}

	${LD} -nostdlib -o ${BIN} ${OFORMAT} ${BUILD_ROOT}/res.bin.o ${BUILD_ROOT}/btcpu/platform/chip/${CT_ASIC}/btcpu/obj/${CT_RELEASE}/btcpu_flash.o \
		--script ${LD_FILE} \
		-Map ${MAP}
endif
endif
...
############################ hex files #######################################
# Remove sections should not linked into bin 
SECTION_REMOVE = --remove-section .internal_rom --remove-section .sys_rom  --remove-section .bt_introm_data
SECTION_REMOVE += --remove-section .rom_inter_data --remove-section .boot_rom_version_number  
SECTION_REMOVE += --remove-section .internal_rom.crc --remove-section .btcpu_rom --remove-section .bt_inter_rom.crc
...
${HEX}: force
	...
	@${ECHO}
ifeq "${COMPILER}" "armcc"
	fromelf --m32 ${AXF_FINAL} --output ${HEX}
else
	$(OBJCOPY)  ${OBJCPY_OPTS} ${SREC} ${BIN} ${HEX}
	${OBJCOPY}  ${SECTION_REMOVE} -O binary ${BIN} ${HEXBIN_FINAL} ${STDERR_NULL}
endif
ifeq "${CHIP_CPU_TYPE}" "arm"
ifeq "${COMPILER}" "gcc"
	${OBJCOPY}  --remove-section .backup --remove-section .fixptr --remove-section .bt_inter_ram -O binary ${BIN_FINAL} ${HEXBIN_FINAL} ${STDERR_NULL}
	${OBJDUMP} -D ${BIN_FINAL} > ${ASM_FINAL}
endif
endif
...
ifeq "${BTCPUGEN}"  "yes"
	${MAKE} dumpstem BUILD_BTCPU_IMAGE=1
endif
ifeq "${RAMINITGEN}"  "yes"
...
FINAL_IMAGE_FILE := ${LODBASE}modemTotal.img
...
ifeq "${RAMINITGEN}"  "yes"
	...
	@${ECHO}
	@${ECHO} "CAT               $(notdir ${FINAL_IMAGE_FILE})"
	cat ${RAMINIT_SREC_IMAGE_FILE} ${SREC_IMAGE_FILE} > ${FINAL_IMAGE_FILE}
	@${ECHO}
	@${ECHO} "---------------------------------------------------"
	@${ECHO} "$(notdir ${FINAL_IMAGE_FILE}) Generated"
	@${ECHO} "---------------------------------------------------"
...
ifneq "${COMPILER}" "armcc"	
			${OBJDUMP} -D --start-address=0x80000000 ${BIN_FINAL} > ${ASM_FINAL}
endif
			@${ECHO}
			@${ECHO} "---------------------------------------------------"
...
all: $(TOP_TARGET)
ifneq ("${IS_ENTRY_POINT}", "yes")
ifeq ("${IS_MODULE_LIB}", "yes")
	@${ECHO} "CP                library to platform/lib/${CT_ASIC}"
	mkdir -p ${SOFT_WORKDIR}/platform/lib/${CT_ASIC}/
ifneq	("${LIBRARY_NAME}", "")
	cp -f ${LOCAL_SRCLIBRARY} ${SOFT_WORKDIR}/platform/lib/${CT_ASIC}/lib${LIBRARY_NAME}_${CT_RELEASE}.a
else
	cp -f ${LOCAL_SRCLIBRARY} ${SOFT_WORKDIR}/platform/lib/${CT_ASIC}/
endif	
endif
endif
